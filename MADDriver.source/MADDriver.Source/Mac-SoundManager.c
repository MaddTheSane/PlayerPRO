...
//	else
	...
	{
		#if CALL_NOT_IN_CARBON
		doubleBuffer = (SndDoubleBufferPtr) MADNewPtrClear( sizeof (PPSndDoubleBuffer) + inMADDriver->BufSize + 20L, inMADDriver->lib);
		#else
		doubleBuffer = (PPSndDoubleBufferPtr) MADNewPtrClear( sizeof (PPSndDoubleBuffer) + inMADDriver->BufSize + 20L, inMADDriver->lib);
		#endif
