...
static OSErr MED_Load(Ptr	theMED, long MEDSize, MADMusic *theMAD, MADDriverSettings *init)
{
	...
	mystrcpy(theMAD->header->infos, "\pConverted by PlayerPRO MED Plug (\xA9\x41ntoine ROSSET <rossetantoine@bluewin.ch>)");
	...
	switch (mh->id) {
		case 'MMD0':
			if (LoadMMD0Patterns(theMAD, theMED, init) != noErr)
				return MADUnknowErr;
			break;
			
		case 'MMD1':
			if (LoadMMD1Patterns(theMAD, theMED, init) != noErr)
				return MADUnknowErr;
			break;
	}
...
static inline Boolean compMem(void *a, void *b, long s)
{
