...
BOOL MED_Init( MADDriverSettings *init)
{
	mh=NULL;
	ms=NULL;
	ba=NULL;        // blockarr
...
void EffectCvt( UBYTE eff, UBYTE dat, Cmd *aCmd)
{
	switch(eff){
...
void MED_Convert1( short col, short patID, MADMusic *theMAD)
{
	int t;
...
void MED_Convert0( short patID, MADMusic *theMAD)
{
	int 		t, zz;
...
OSErr LoadMMD0Patterns( MADMusic *theMAD, Ptr theMED, MADDriverSettings *init)
{
	int 		t,row,col;
...
OSErr LoadMMD1Patterns( MADMusic *theMAD, Ptr theMED, MADDriverSettings *init)
{
	int t,row,col;
...
OSErr MED_Load( Ptr	theMED, long MEDSize, MADMusic *theMAD, MADDriverSettings *init)
{
	int 		t, i;
	ULONG 		sa[64];
	InstrHdr 	s;
	...
	if( theMAD->header == 0L) DebugStr("\pHeader: I NEED MEMORY !!! NOW !");
...
OSErr ExtractMEDInfo( PPInfoRec *info, Ptr theMED)
{
	...
	MADstrcpy( info->internalFileName, "");
	...
	MADstrcpy( info->formatDescription, "MED Plug");
