...
static OSErr PPConvertMod2Mad( Ptr aMOD,long MODSize, MADMusic	*theMAD, MADDriverSettings* init)
{
	...
	strcpy( theMAD->header->infos, (Ptr) "Converted by PlayerPRO UMX Plug (\xA9 Antoine ROSSET <rossetantoine@bluewin.ch>)");
...
static OSErr mainUMX( OSType order, Ptr AlienFileName, MADMusic *MadFile, PPInfoRec *info, MADDriverSettings *init)
{
	OSErr		myErr = noErr;
	Ptr			AlienFile;
	long		sndSize;
...
{
	return mainUMX(order, AlienFileName, MadFile, info, init);
}
#endif
