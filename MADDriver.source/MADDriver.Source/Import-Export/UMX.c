...
OSErr PPConvertMod2Mad( Ptr aMOD,long MODSize, MADMusic	*theMAD, MADDriverSettings* init)
{
	...
	MADstrcpy( theMAD->header->infos, (Ptr) "Converted by PlayerPRO MOD Plug (\251Antoine ROSSET <rossetantoine@bluewin.ch>)");
...
OSErr ExtractUMXInfo( PPInfoRec *info, Ptr AlienFile)
{
	long	PatternSize;
	short	i;
	short	maxInstru;
	...
	MADstrcpy( info->formatDescription, "UMX Plug");
	
	return noErr;
}

OSErr TestUMXFile( Ptr AlienFile, long EOFo)
{
	short		maxInstru;
	long		PatternSize, i, ;
	short		tracksNo;
