...
OSErr LoadMADH( Ptr MADPtr, MADMusic *MadFile, MADDriverSettings *init)
{
short 					i, maxInstru;
...
if( MadFile->header == 0L) return MADNeedMemory;
...
/**** PARTITION ****/
for( i = MadHeader->numPat; i < MAXPATTERN; i++) MadFile->partition[ i] = 0L;
...
for( i = 0; i < MadHeader->numPat; i++)
{
	...
	if( MadFile->partition[ i] == 0L) return MADNeedMemory;
...
short	i;
	...
	MADstrcpy( info->internalFileName, MADPtr->name);
	
	MADstrcpy( info->formatDescription, "MAD Resource (APPL)");
...
OSErr SaveAPPL( short APPLType, short fRefNum, MADMusic *MadFile, MADDriverSettings *init)
{
OSErr					iErr;
short					i, x;
...
Boolean HasToClose( FSSpec *spec)
{
