...
Ptr	ConvertMad2S3M( MADMusic *theMAD, MADDriverSettings *init, long *sndSize)
{
	long 				i, x, z;
...
OSErr ConvertS3M2Mad( Ptr	theS3M, long size, MADMusic *theMAD, MADDriverSettings *init)
{
	...
	if( s3minfo.orders == 0L) return MADNeedMemory;
	...
	if( s3minfo.parapins == 0L) return MADNeedMemory;
	...
	if( s3minfo.insdata == 0L) return MADNeedMemory;
	for (i = 0; i < s3minfo.insnum; i++)
	{
		theInstrument[ i] = 0L;
...
OSErr ExtractS3MInfo( PPInfoRec *info, Ptr AlienFile)
{
	...
	myS3M->name[ 27] = '\0';
	MADstrcpy( info->internalFileName, myS3M->name);
	...
	MADstrcpy( info->formatDescription, "S3M Plug");
