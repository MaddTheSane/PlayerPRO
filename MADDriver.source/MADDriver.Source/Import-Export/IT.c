...
inline Byte _mm_read_UBYTE( Ptr	*samplePtr)
{
...
inline unsigned short ReadUS( Ptr	*samplePtr)
{
...
long read_itcompr8(ITPACK* status,Ptr *reader,Byte *sl_buffer,unsigned short count,unsigned short* incnt)
{
...
long read_itcompr16( ITPACK *status,Ptr *reader,short *sl_buffer,unsigned short count,unsigned short* incnt)
{
...
OSErr DecompressSample( short bits, Ptr reader, long length, Ptr destPtr)		// sloader.c
{
	int				stodo,t,u;
...
Boolean ITcompMem( Ptr a, Ptr b, long s)
{
long 	i;
...
void ConvertITEffect( Byte B0, Byte B1, Byte *Cmd, Byte *Arg, short channel)
{
	Byte		LoB1 = LOW( B1);
	Byte		HiB1 = HI( B1);
...
//	if( ITinfo.insNum > 0) DebugStr("\pInsNum");
	...
	mystrcpy( theMAD->header->infos, (Ptr) "\pConverted by PlayerPRO IT Plug (\251Antoine ROSSET <rossetantoine@bluewin.ch>)");
	...
	if(ITinfo.orders)	DisposePtr( (Ptr) ITinfo.orders);
	if(ITinfo.parapins)	DisposePtr( (Ptr) ITinfo.parapins);
	if(ITinfo.parappat)	DisposePtr( (Ptr) ITinfo.parappat);
	if(ITinfo.insdata)	DisposePtr( (Ptr) ITinfo.insdata);
	if(ITinfo.parapsamp)DisposePtr( (Ptr) ITinfo.parapsamp);
	if(ITinfo.sampdata)	DisposePtr( (Ptr) ITinfo.sampdata);
	return noErr;
}
