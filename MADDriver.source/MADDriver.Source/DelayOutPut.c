...
inline void PrepareInline( long *VolInter, long* rVolInter, double p2, double v1, double v2)
{
double temp;
...
short	i;
	...
	switch( intDriver->DriverSettings.outPutBits)
	{
		case 16:
			...
			for( i = 0; i < MAXCHANEFFECT; i++)
			{
				if( intDriver->DASCEffectBuffer[ i] != 0L) DisposePtr( (Ptr) intDriver->DASCEffectBuffer[ i]);
				intDriver->DASCEffectBuffer[ i] = 0L;
			}
		break;
...
void Sampler16AddDelay( Channel *curVoice, long	*ASCBuffer, MADDriverRec *intDriver)
{
	...
	finalperiod = ((long long) finalperiod * (long long) intDriver->FreqExt) / (long long) 8000L;
