...
MADMusic* CreateFreeMADK( void)
{
	...
	for(i=0; i< MADI->numPat; i++)
	{
		...
		music->partition[ i]->header.patBytes 	= 0L;
		music->partition[ i]->header.unused2 		= 0L;
	...
	for( i = music->header->numPat; i < MAXPATTERN ; i++) music->partition[ i] = 0L;
	...
	for( i = music->header->numPat-1; i >= 0 ; i--)
	{
		...
		else if( music->partition[ i]->header.size > 64)
		{
			...
			while( patsize < srcPat->header.size)
			{
				...
				for( x = music->header->numPat; x < MAXPATTERN; x++) music->partition[ x] = 0L;
...
/****************************/
	...
	if( NewSoundManager == true)
	{
		...
		if( NewSoundManager31)
		{
			...
			iErr = GetSoundOutputInfo( 0L, siSampleRate, &Init->outPutRate);			if(iErr) goto oldWay;
			iErr = GetSoundOutputInfo( 0L, siSampleSize, (void*) &Init->outPutBits);	if(iErr) goto oldWay;
		}
		else
		#endif
		{
	...
	if( intDriver->DriverSettings.Reverb)
	{
		...
		switch( intDriver->DriverSettings.outPutBits)
		{
			case 8:
				...
				if( intDriver->ReverbPtr == 0L) return MADNeedMemory;
...
void MADDisposeDriverBuffer( MADDriverRec *intDriver)
{
	...
	for( i = 0; i < MAXDRAW; i++)
	{
		if( intDriver->OsciDrawPtr[ i] != 0L)	DisposePtr( intDriver->OsciDrawPtr[ i]);
		intDriver->OsciDrawPtr[ i] = 0L;
	}

	if( intDriver->IntDataPtr != 0L)	DisposePtr( intDriver->IntDataPtr);
	intDriver->IntDataPtr = 0L;
}
...
OSErr MADCreateDriverBuffer( MADDriverRec *intDriver)
{
	...
	for( i = 0; i < MAXDRAW; i++)
	{
		...
		if( intDriver->OsciDrawPtr[ i] == 0L) return MADNeedMemory;
	}
	
	intDriver->IntDataPtr = MADNewPtr( BufSize, intDriver->lib);
	if( intDriver->IntDataPtr == 0L) return MADNeedMemory;
...
OSErr MADCreateTiming( MADDriverRec *intDriver)
{
	...
	intDriver->VSYNC	= ((intDriver->DriverSettings.outPutRate>>16) * 125L * intDriver->DriverSettings.oversampling) / (50L);
...
OSErr MADChangeDriverSettings( MADDriverSettings	*DriverInitParam, MADDriverRec** returnDriver)
{
	...
	if( music)
	{
		MADAttachDriverToMusic( *returnDriver, music, 0L);
	...
	*returnDriver = 0L;
	...
	switch( MDriver->DriverSettings.driverMode)
	{
		...
		case DirectSound95NT:
			MDriver->ASCBUFFER = 7500L * MDriver->DriverSettings.oversampling;
		break;
	...
	switch( MDriver->DriverSettings.driverMode)
	{
		...
		/*	MDriver->gOutNodeRefNum = -1;
			OMSAddPort( 'SNPL', 'out ', omsPortTypeOutput, NULL, 0L, &MDriver->MIDIPortRefNum);*/
...
{
	if( MDriver->IntDataPtr == 0L) return noErr;
...
OSErr MADInitLibrary( FSSpec *PlugsFolderName, Boolean sysMemory, MADLibrary **lib)
{
	...
	*lib = (MADLibrary*) NewPtrClear( sizeof( MADLibrary));
	...
	if( *lib == 0L) return MADNeedMemory;
	
	(*lib)->IDType = 'MADD';
	(*lib)->sysMemory = sysMemory;
...
OSErr MADLoadMusicRsrc( MADMusic **music, OSType IDName, short	IDNo)
{
	...
	if( theRsrc == 0L) return MADReadingErr;
	
	theErr = MADReadMAD( music, 0, MADRsrcType, theRsrc, 0L);
	if( theErr != noErr)
	{
...
//	MADDisposeMusic( music);
	...
	theErr = MADReadMAD( music, 0, MADPtrType, 0L, myPtr);
	if( theErr != noErr) return theErr;
...
//	MADDisposeMusic( music);
	...
	if( srcFile == 0L) return MADReadingErr;
	
	theErr = MADReadMAD( music, srcFile, MADFileType, 0L, 0L);
	if( theErr != noErr)
	{
		iClose( srcFile);
		//MADDisposeMusic( music, MADDriver);
...
FSSpec					saved;
	...
	HGetVol( 0L, &saved.vRefNum, &saved.parID);
	HSetVol( 0L, theSpec->vRefNum , theSpec->parID);
	MYP2CStr( theSpec->name);
	
	if( !MADstrcmp( "MADK", plugType)) 	iErr = MADLoadMADFileCString( music, (Ptr) theSpec->name);
	else								iErr = MADLoadMusicFileCString( lib, music, plugType, (Ptr) theSpec->name);
	
	MYC2PStr( (Ptr) theSpec->name);
	HSetVol( 0L, saved.vRefNum , saved.parID);
...
OSErr MADCopyCurrentPartition( MADMusic *aPartition)
{
	...
	if( MDriver->header == 0L) return -1;
	if( aPartition == 0L) return -1;
	
	aPartition->header = (MADSpec*) MADNewPtr( sizeof( MADSpec), &MDriver->DriverSettings);
	if( aPartition->header == 0L) return -1;
	...
	for( i = 0; i < MDriver->header->numPat ; i++)
	{
		...
		if( aPartition->partition[ i] == 0L) return -1;
		
		BlockMoveData( MDriver->partition[ i], aPartition->partition[ i], GetPtrSize( (Ptr) MDriver->partition[ i]));
	}
	for( i = MDriver->header->numPat; i < MAXPATTERN ; i++) aPartition->partition[ i] = 0L;
...
OSErr MADSetMusicStatus( MADDriverRec *MDriver, long minV, long maxV, long curV)
{
	...
	if( MDriver->curMusic == 0L)
	{
		return -1; 
	}
	
	if( MDriver->curMusic->header == 0L)
	{
		return -1; 
	}
	...
	for( i = 0; i < MDriver->curMusic->header->numPointers; i++)
	{
		for( x = 0; x < MDriver->curMusic->partition[ MDriver->curMusic->header->oPointers[ i]]->header.size; x++)
		{
			...
			if( dstTime <= timeResult + ((float) (time * 125L * speed * 60L)) / ((float) (50L * finespeed)))
			{
				MDriver->PL = i;
...
switch( InPutType)
{
	...
	case MADRsrcType:
		OffSetToSample = 0L;
		ReadPartialResource( MADRsrc, 0L, MDriver->header, inOutCount);
...
/**** PARTITION ****/
for( i = MDriver->header->numPat; i < MAXPATTERN; i++) MDriver->partition[ i] = 0L;
...
for( i = 0; i < MDriver->header->numPat; i++)
{
	...
	if( MDriver->partition[ i] == 0L)
	{
		for( x = 0; x < i; x++)
		{
			if( MDriver->partition[ x] != 0L)	DisposePtr( (Ptr) MDriver->partition[ x]);
		}
		DisposePtr( (Ptr) MDriver->header);
		DisposePtr( (Ptr) MDriver);
...
OSErr MADKillSample( MADMusic *MDriver, short ins, short sample)
{
	...
	if( MDriver->sample[ ins * MAXSAMPLE + sample] == 0L) return -1;
	...
	MDriver->sample[ ins * MAXSAMPLE + sample] = 0L;
...
{
	if( music == 0L) return 0;
	if( intDriver == 0L) return 0;
...
//	if( (*music)->currentDriver != 0L)
	...
	if( MDriver)
	{
		if( MDriver->curMusic == *music) MDriver->curMusic = 0L;
	}
	...
	if(  (*music)->header != 0L)
	{
		...
		{
			if(  (*music)->partition[ x] != 0L) DisposePtr( (Ptr)  (*music)->partition[ x]);
			 (*music)->partition[ x] = 0L;
		}
		
		for( x = (*music)->header->numPat; x < MAXPATTERN ; x++)
		{
			if(  (*music)->partition[ x] != 0L) DebugStr("\pMADDispose (*music) !");
		}
		...
		if(  (*music)->fid == 0L) DebugStr("\pMADDispose (*fid) !");
		DisposePtr( (Ptr) (*music)->fid);
		
		if(  (*music)->sample == 0L) DebugStr("\pMADDispose (*sample) !");
		DisposePtr( (Ptr) (*music)->sample);
		
		if(  (*music)->sets == 0L) DebugStr("\pMADDispose (*sets) !");
...
OSErr MADPlaySoundDataSYNC( MADDriverRec *MDriver, Ptr soundPtr, long size, long channel, long note, long amplitude, long loopBeg, long loopSize, unsigned long rate, Boolean stereo)
{
	...
	if( iErr == noErr)
	{
		...
		while( continueLoop)
		{
			...
			if( MDriver->chan[ channel].samplePtr == 0L) continueLoop = false;
		...
		if( MDriver->chan[ channel].samplePtr != 0L)
		{
			MDriver->chan[ channel].curPtr 		= MDriver->chan[ channel].maxPtr;
			MDriver->chan[ channel].samplePtr	= 0L;
...
PatData* CompressPartitionMAD1( MADMusic *MDriver, PatData* myPat)
{
	...
	if( finalPtr == 0L) DebugStr("\pCompressPartitionMAD1");
...
void DMAIntRemove( unsigned long vectorBit)
{
	...
	*((long*)( 0x08L + (long) dstPtr + vectorBit*8L)) = 0L;
}
...
//	THz 			savedZone;
	...
	MDriver->VBL.VBL.qLink		= 0L;		// VLBQueue
...
void InstallPersistentVBL( MADDriverRec *MDriver, VBLTask *theVBLRec)
{
	...
	if( MDriver->SysHeapPtr == 0L) DebugStr("\pNewPtrSys Error");
