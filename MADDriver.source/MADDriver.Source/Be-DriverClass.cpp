...
extern "C" {
Boolean	DirectSave( Ptr 				myPtr,
					MADDriverSettings	*driverType,
...
MADDriverSettings	MADDriverClass::CreateDefaultDriver( void )
{
...
bool	MADDriverClass::InitLibrary( MADDriverSettings *init)
{
	...
	if ( (libraryError = MADInitLibrary( "add-ons", init->sysMemory, &MADLib)) != noErr )
	{
	...
	format.frame_rate = ((float) init->outPutRate / (float) 0x0000FFFF);
...
bool	MADDriverClass::LoadMusic( entry_ref* ref, OSType type, bool playIt )
{
	...
	libraryError = MADMusicIdentifyCString( MADLib, plugName, (Ptr)filePath.Path());
	if( libraryError == noErr)
	{
		libraryError = MADLoadMusicFileCString( MADLib, &curMusic, plugName, (Ptr)filePath.Path() );
		if( libraryError == noErr)
		{
			MADAttachDriverToMusic( curDriverRec, curMusic );
			if ( playIt ) StartMusic();
		}
	}
		...
		case B_AUDIO_FLOAT:
			...
			switch( curMADDriver->DriverSettings.outPutBits)
			{
				case 16:
					...
					for( i = 0; i < count/sizeof( float); i++) floatbuffer[ i] = ( (float) shortbuffer[ i] * 1.0) / 32767.0;
				break;
				
				case 8:
					Byte	*charbuffer = (Byte*) curMADDriver->IntDataPtr;
					for( i = 0; i < count/sizeof( float); i++) floatbuffer[ i] = ( ((float) charbuffer[ i]- 0x80) * 1.0) / 127.0;
				break;
			}
		break;
		
		default:
			return;		// SUPPORTS ONLY the float format! BeOS driver works in float.
		break;
	}
}
