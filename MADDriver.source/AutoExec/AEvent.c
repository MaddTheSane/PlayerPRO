...
pascal OSErr  MyHandleQDoc (AppleEvent *theAppleEvent, AppleEvent *reply, long handlerRefCon);
pascal OSErr  MyHandleQApp (AppleEvent *theAppleEvent, AppleEvent *reply, long handlerRefCon);
/********************************/
/********************************/
...
pascal OSErr  MyHandleQApp (AppleEvent *theAppleEvent, AppleEvent *reply, long
														handlerRefCon)
{
	AEDescList	docList;
	OSErr		err;
...
short InstallAE(void)
{
	...
	if (aEvents)
	{
		err = AEInstallEventHandler (kCoreEventClass, kAEOpenDocuments, NewAEEventHandlerProc( MyHandleODoc),0, false);
		err = AEInstallEventHandler (kCoreEventClass, kAEQuitApplication, NewAEEventHandlerProc( MyHandleQApp),0, false);
	}
