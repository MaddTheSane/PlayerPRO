...
short PressTypeMenu( short whichMenu, short item)
{
	...
	{
		case 128:
			if( !Stereo) DisableItem( MenuDriver, 2);
	...
	if( HiWord( mresult) != 0)
	{
		...
		MADAttachDriverToMusic( MADDriver, music);
		...
		else if( theEvent.what == mouseDown)
		{
			switch( thePart = FindWindow( theEvent.where, &whichWindow))
			{
				...
				case inZoomOut:
					...
					{
						if( whichWindow->portRect.bottom > 29) SizeWindow( whichWindow, whichWindow->portRect.right, 29, true);
						else SizeWindow( whichWindow, whichWindow->portRect.right, 100, true); 
					}
				break;
			}
		}
...
oldTempL = -100;
OnTourne = true;
MADInitLibrary("Plugs", true, &MADLib);
...
SizeWindow( myDialog, myDialog->portRect.right, 29, true);
ShowWindow( myDialog);
SetPort( myDialog);
