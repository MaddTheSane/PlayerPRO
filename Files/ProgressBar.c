...
//	if( WorkTotal < MINIMUS) return;
	...
	theProgressDia = GetNewDialog( 156, 0L, (WindowPtr) -1L);
...
//	if( WorkTotal < MINIMUS) return false;
	...
	if( theProgressDia == 0L) return false;
...
//	thePrefs.DirectDriverType = MADDriver->DriverSettings;
	...
	MADAttachDriverToMusic( copyDriver, copyMusic, 0L);
	...
	if( sndPtr == 0L) MyDebugStr( __LINE__, __FILE__, "Memory Error, Increase Memory");
	
	outSound = NewPtrClear( (copyDriver->ASCBUFFERReal*Packet*2L*thePrefs.amplitude) / 8L);
	if( outSound == 0L) MyDebugStr( __LINE__, __FILE__, "Memory Error, Increase Memory");
	
	compSound = NewPtrClear( (copyDriver->ASCBUFFERReal*Packet*2L*thePrefs.amplitude) / 8L);
	if( compSound == 0L) MyDebugStr( __LINE__, __FILE__, "Memory Error, Increase Memory");
...
/*	iErr = SoundConverterGetBufferSizes(sc, 1024, &inputFrames, &inputBytes, &outputBytes);
	...
	//	SetResLoad( false);
		
		{
			...
			sndHandle = NewHandle( 20000L);
			if( sndHandle == 0L) {MyDebugStr( __LINE__, __FILE__, "Need MEMORY");	CloseWorkingWindow(); return false;}
...
void StopAIFFExporting( void)
{
	...
	if( theType == 'sfil')
	{
		...
		sndHandle2 = NewHandle( 20000L);
		if( sndHandle2 == 0L) {MyDebugStr( __LINE__, __FILE__, "Need MEMORY");	return;}
	...
	switch( patternID)
	{
		case -2:
		{
			...
			copyMusic->partition[ copyMusic->header->numPat] = 0L;
		}
	}
