...
void InitUndo(void)
{
	...
	for( i = 0; i < MAXINSTRU; i++) CopyAllInstruments[ i] = 0L;
	for( i = 0; i < MAXPATTERN; i++) CopyPartition[ i] = 0L;
...
void ResetUndo(void)
{
	...
	if( myUndo.data != 0L) MyDisposePtr( & myUndo.data);
	myUndo.data = 0L;
	myUndo.dataSize = 0L;
	...
	{
		if( CopyAllInstruments[ i] != 0L) MyDisposePtr( & CopyAllInstruments[ i]);
		CopyAllInstruments[ i] = 0L;
	}
	
	for( i = 0; i < MAXPATTERN; i++)
	{
		if( CopyPartition[ i] != 0L) MyDisposePtr( (Ptr*) &CopyPartition[ i]);
		CopyPartition[ i] = 0L;
	}
...
void SaveUndo( short UndoType, short ID, Str255 textMenu)
{
	...
	switch( UndoType)
	{
		...
		case USample:
			...
			else
			{
				...
				for( i = 0; i < CopyFid.numSamples; i++)
				{
					...
					if( curData == 0L) MyDebugStr( __LINE__, __FILE__, "BIG UNDO Error 1");
					if( curData->size > 0)
					{
						if( curData->data == 0L) MyDebugStr( __LINE__, __FILE__, "BIG UNDO Error 2A");
		...
		case UAllPatterns:
			...
			myUndo.data = MyNewPtr( myUndo.dataSize);
			if( myUndo.data == 0L) { ResetUndo();	break;}
				...
				for( i = 0; i < MAXINSTRU; i++)
				{
					...
					if( CopyAllInstruments[ i] != 0L)
					{
						...
						for( x = 0; x < curMusic->fid[ i].numSamples; x++)
						{
							...
							if( curData->size > 0)
							{
								if( curData->data == 0L) MyDebugStr( __LINE__, __FILE__, "BIG UNDO Error 2");
			...
			if( UndoType == UAllPatterns)
			{
				for( i = 0; i < MAXPATTERN; i++)
				{
					...
					if( i >= curMusic->header->numPat) if( curMusic->partition[ i] != 0L) MyDebugStr( __LINE__, __FILE__, "Check Partition-Partition UNDO.");
				
					if( curMusic->partition[ i] != 0L)
					{
		...
		default:
			MyDebugStr( __LINE__, __FILE__, "Unknow UNDO");
		break;
	}
	
	EnableMenuItem( EditMenu, 1);
	SetItem( EditMenu, 1, textMenu);
}
...
Boolean				IsReading;
	...
	switch( myUndo.UndoType)
	{
		...
		case UHeader:
			...
			/** REDO **/
			myTempUndo = MyNewPtr( myUndo.dataSize);
			if( myTempUndo == 0L) MyDebugStr( __LINE__, __FILE__, "UndoError - MEMORY");
			...
			if( myUndo.UndoType == UAllSamples)
			{
				for( i = 0; i < MAXINSTRU; i++)
				{
					...
					if( CopyAllInstruments[ i] != 0L)
					{
						...
						MyDisposePtr( & CopyAllInstruments[ i]);	CopyAllInstruments[ i] = 0L;
					}
			...
			if( myUndo.UndoType == UAllPatterns)
			{
				for( i = 0; i < MAXPATTERN; i++)
				{
					...
					if( curMusic->partition[ i] != 0L)
					{
						...
						MyDisposePtr( (Ptr*) &curMusic->partition[ i]);	curMusic->partition[ i] = 0L;
					}
					else myTempInstru = 0L;
					/********/
