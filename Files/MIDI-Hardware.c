...
void MyNullHook()
{
	...
	if( pkt->data[ 0] >= 0x90 && pkt->data[ 0] <= 0x9F)							// NOTE ON
	{
		...
		if( myNote >= 0 && myNote < NUMBER_NOTES)
		{
			...
			else if( oldWindow == GetDialogWindow( EditorDlog)) DigitalEditorProcess( myNote, 0L, 0L, 0L);
...
OSErr	InitOMS(OSType appSignature, OSType inPortID, OSType outPortID)
{
	...
	gOutputPortRefNum = -1;
	err = OMSAddPort( appSignature, outPortID, omsPortTypeOutput, NULL, 0L, &gOutputPortRefNum);
...
void SelectOMSConnections( Boolean Input)
{
	...
	if( Input)
	{
		...
		in = OMSChooseNodes( prevSelectionIN, "\pSelect Input", false, omsIncludeInputs + omsIncludeReal + omsIncludeVirtual + omsIncludeSync, 0L);
		if( in)
		{
	...
	else
	{
		...
		out = OMSChooseNodes( prevSelectionOUT, "\pSelect Output", false, omsIncludeOutputs + omsIncludeReal + omsIncludeVirtual, 0L);
		if( out)
		{
...
void SendMIDITimingClock( MADDriverRec *MDriver)
{
	...
	if( MDriver == 0L) Debugger();
	if( MDriver->curMusic == 0L) Debugger();
	if( MDriver->curMusic->header == 0L) Debugger();
