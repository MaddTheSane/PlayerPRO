	...
	if( selectedControl != 0L)	// Drag sur un bouton !!!!
	{
		...
		if( selectedControl == OpenBut)
		{
			...
			/*	MADDriver->Pat = theCell.v;
				...
				if( ClassicDlog != 0L) SelectWindow2( GetDialogWindow( ClassicDlog));
...
Boolean DragPatternFunction( RgnHandle myRgn, short theNo, EventRecord *theEvent)
{
	...
	AddDragItemFlavor( theDrag, 1, 'VCT2', 0L, 0, 0);
	...
	for( theCell.v = (*PatList2)->dataBounds.bottom-1; theCell.v >= 0; theCell.v--)
	{
		if( curMusic->header->numPat > 1)
		{
			if( LGetSelect( false, &theCell, PatList2))
			{
				...
				curMusic->partition[ theCell.v] = 0L;
			
				for( i = theCell.v; i < curMusic->header->numPat; i++)
				{
					curMusic->partition[ i] = curMusic->partition[ i + 1];
				}
				for( i = curMusic->header->numPat; i < MAXPATTERN; i++) curMusic->partition[ i] = 0L;
...
void UpdatePatListInfo(void)
{
	...
	LDoDraw( false, PatList2);
	SetPatList2( PatList2);
	LDoDraw( true, PatList2);
...
Boolean DialogPatternInfo( short thePos)
{
	...
	TheDia = GetNewDialog( 167,0L, (WindowPtr) -1L);
...
void DoReplace(void)
{
	...
	TheDia = GetNewDialog( 152,0L, (WindowPtr) -1L);
	SetPortDialogPort( TheDia);
	...
	if( itemHit == 1)
	{
		...
		{
			Erreur( 43, -6);	SelIText( TheDia, 9, 0, 32767);
			goto OnRepart;
		}
...
void DialogTracks(void)
{
	...
	TheDia = GetNewDialog( 154,0L, (WindowPtr) -1L);
	SetPortDialogPort( TheDia);
	AutoPosition( TheDia);
	ChangeDialogFont( TheDia);
...
void DoItemPressPatList( short whichItem, DialogPtr whichDialog)    			/* Item hit ID to pass to Dialog function */
{
		...
		/*	case 1:
			...
			case 3:
			if( MyTrackControl( LoadBut, theEvent.where, 0L))
			{
			...
			break;
			case 7:
			if( MyTrackControl( NewBut, theEvent.where, 0L))
			{
...
void SetPatList2( ListHandle	theList)
{
		...
		MyDisposePtr( & mainStr);	mainStr = 0L;
		MyDisposePtr( & stemp);		stemp = 0L;
...
void CreatePatListWindow(void)
{
	...
	PatListDlog = GetNewDialog( 144, 0L, GetDialogWindow( ToolsDlog));
...
if( DragManagerUse)
{
		...
		InstallTrackingHandler( (DragTrackingHandlerUPP) MyTrackingHandlerUPP, GetDialogWindow( PatListDlog), (void *) 0L);
		InstallReceiveHandler( (DragReceiveHandlerUPP) MyReceiveDropHandlerUPP, GetDialogWindow( PatListDlog), (void *) 0L);
}
...
//		LDispose( PatList2);	PatList2 = 0L;
		DisposDialog( PatListDlog);
	}
	PatListDlog = 0L;
...
void COPYPatList(void)
{
	...
	if( LGetSelect( true, &theCell, PatList2))
	{
		...
		ZeroScrap();
		PutScrap( (long) GetPtrSize( (Ptr) curMusic->partition[ theCell.v]), 'PATT', (Ptr) curMusic->partition[ theCell.v]);
...
OSErr				anErr;
	...
	lCntOrErr = GetScrap( 0L, 'PATT', &scrapOffset);
	...
	if( lCntOrErr > 0)
	{
		...
		GetScrap( theHandle, 'PATT', &scrapOffset);
