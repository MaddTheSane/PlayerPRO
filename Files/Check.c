...
Boolean OpenableFile( OSType theType, FSSpec *file)
{
	...
	HSetVol( 0L, file->vRefNum, file->parID);
...
PatData*				PatMAD;
	...
	if( tempPtr == 0L) MyDebugStr( __LINE__, __FILE__, "Memory !!!");
								...
								Boolean	MPG4)
{
	...
	GetDigitalSelection( 0L,0L,0L,0L,&currentPattern);
	
	aDialog = GetNewDialog( 160, 0L, (WindowPtr) -1L);
...
char						theTypePtr[ 5];
	...
	HSetVol( 0L, newFile->vRefNum, newFile->parID);
	/***/
