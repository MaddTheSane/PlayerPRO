...
{
	newRect->left = (Alpha - GetCtlValue( theCtl))*CurLarg;
...
//	SizeControl( theCtl, ClassicDlog->portRect.right -13, 16);
	...
	{
		SetCtlMin( theCtl, 0);
		SetCtlMax( theCtl, PatternLength - PatternLength/ZoomLevelPat);
...
Rect				destRect;
	...
	if( Note == true)
	{
		destRect.left = (tt - GetCtlValue( theCtl)) * CurLarg;
...
{ 
		...
		UpdtDialog( ClassicDlog, visibleRegion);
...
void DoItemPressClassic( short whichItem, DialogPtr whichDialog)    			/* Item hit ID to pass to Dialog function */
{
			...
			if(PtInRect( myPt, &NoteZone))
			{
				...
				if( IsPressed( 0x003A))
				{
					...
					if( ZoomLevelPat > 1)
					{
						...
						SetCtlValue( theCtl, GetCtlValue( theCtl) / 2);
						SetControlHClassic();
				...
				else if( IsPressed( 0x0037) || SelectMode == eZoom)
				{
					...
					if( ZoomLevelPat < 32)
					{
						...
						SetCtlValue( theCtl, GetCtlValue( theCtl)*2 + bogus);
		...
		switch( whichItem)
		{
			...
			case 15:
				if( GetControlHilite( ZoomBut) == 0 && MyTrackControl( ZoomBut, theEvent.where, 0L))
				{
					SelectMode = eZoom;
			...
			case 7:
				...
				InsMenuItem( TrackView, "\pAll tracks", 0);
				...
				DelMenuItem( TrackView, 1);
			break;
			
			case 8:
				InsMenuItem( InstruMenu, "\pAll instruments", 0);
...
void CreateClassicWindow(void)
{
	...
	ClassicDlog = GetNewDialog( 147, 0L, GetDialogWindow( ToolsDlog));
	SetWindEtat( GetDialogWindow( ClassicDlog));
	SetPortDialogPort( ClassicDlog);
