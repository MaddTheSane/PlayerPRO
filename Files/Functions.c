...
void SetCtlName(DialogPtr theDialog, short theItem, Str255 title)
{
	...
	SetCTitle( (ControlHandle) aHandle, title);
}
...
*/
	...
	GetDigitalSelection( &DStartTrack, &DStartPos, &DEndTrack, &DEndPos, 0L);
	
	FindDlog = GetNewDialog( 152,0L, (WindowPtr) -1L);
	SetPortDialogPort( FindDlog);
