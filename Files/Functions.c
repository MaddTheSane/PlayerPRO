...
void SetUpButtons(void)
{
	...
	if (MozartDlog == NULL) {
		...
		if (FindWhere == ClassicalSelection)
			FindWhere = CurrentPat;
	}
	...
	{
		case CurrentPat:
			TurnRadio(10, FindDlog, true);
			break;
			
		case DigitalSelection:
			TurnRadio(11, FindDlog, true);
			break;
			
		case ClassicalSelection:
			TurnRadio(13, FindDlog, true);
			break;
			
		case Everywhere:
			TurnRadio(12, FindDlog, true);
			break;
	}
	...
	if (FindTrack == -1)
		TurnRadio(46, FindDlog, true);
	else {
		TurnRadio(43, FindDlog, true);
		NumToString(FindTrack+1, str);
		SetDText(FindDlog, 45, str);
	}
	...
	if (FArguActi)
		TurnRadio(6, FindDlog, true);
	if (FVolActi)
		TurnRadio(39, FindDlog, true);
	...
	if (RArguActi)
		TurnRadio(24, FindDlog, true);
	if (RVolActi)
		TurnRadio(41, FindDlog, true);
}
...
short ExtractFunctionArgu(Str255 aStr)
{
	...
	if (aStr[0] == 1) {
		...
		if (aStr[2] >= 'A' && aStr[2] <= 'F')
			tt = 10 + aStr[2] - 'A';
		else if (aStr[2] >= '0' && aStr[2] <= '9')
			tt = aStr[2] - '0';
		else
			return -1;
...
Boolean SetUpFind()
{
	...
	if (FindWhere == Everywhere) {
		// all patterns
		FStartPat = 0;
		FEndPat = curMusic->header->numPat;
	...
	switch (whichItem) {
		...
		case 13:
			...
			switch (whichItem) {
				...
				case 11:
					FindWhere = DigitalSelection;
					break;
					
				case 13:
					FindWhere = ClassicalSelection;
					break;
					
				case 12:
					FindWhere = Everywhere;
					break;
			}
			break;
			/***/
		...
		case 43:
			...
			switch (whichItem) {
				...
				case 43:
					GetDText(FindDlog, 45, theStr);
					StringToNum(theStr, &FindTrack);
					FindTrack--;
					break;
			}
			break;
		...
		case 16:		// Note menu
			...
			tempInstru = *shortPtr;
			if (tempInstru == 0xFF)
				tempInstru = NUMBER_NOTES;
			if (tempInstru == 0xFE)
				tempInstru = NUMBER_NOTES + 1;
			...
			if (HiWord(mresult) != 0) {
				*shortPtr = LoWord(mresult)-1;
				...
				if (*shortPtr == NUMBER_NOTES)
					*shortPtr = 0xFF;
				if (*shortPtr == NUMBER_NOTES + 1)
					*shortPtr = 0xFE;
		...
		case 1:		// FIND
			if (FindModify == true) {
				if (FindInternal(Find))
					FindModify = false;
				else
					FindModify = true;
			} else {
				if (!FindInternal(Next))
					FindModify = true;
			}
			break;
