					  ...
					  FSSpec					*spec)
{
	...
	//	HSetVol( NULL, ThePPINPlug[ PlugNo].file.vRefNum, ThePPINPlug[ PlugNo].file.parID);
	fileID = FSpOpenResFile( &ThePPINPlug[ PlugNo].file, fsCurPerm);
	
	thePPInfoPlug.fileType = ThePPINPlug[ PlugNo].type;
	
	myErr = GetDiskFragment( &ThePPINPlug[ PlugNo].file, 0, kCFragGoesToEOF, ThePPINPlug[ PlugNo].file.name, kLoadCFrag, &connID, (Ptr *) &mainPLUG, errName);
...
void LoadPPINPLUG(	short	No, StringPtr	theName)
{
	...
	HGetVol( NULL, &ThePPINPlug[ No].file.vRefNum, &ThePPINPlug[ No].file.parID);
...
pascal short PPINDlgHook( short item, DialogPtr theDialog, void *myDataPtr)
{
	...
	{	
		case getOpen:
		{
			...
			else HSetVol( NULL, IntReply->sfFile.vRefNum, IntReply->sfFile.parID);
		}
			break;
...
static PPInstrumentPlugin** PPINLoadPlug(CFBundleRef theBundle)
{
	CFPlugInRef				plugToTest = CFBundleGetPlugIn(theBundle);
	CFArrayRef				factories = NULL;
	Boolean					foundInterface = FALSE;
	PPInstrumentPlugin		**formatPlugA = NULL;
...
#pragma mark This is where we add the plug to the plug library.
					...
					ThePPINPlug[tPlug].xxxx = tempMADPlug;
					ThePPINPlug[tPlug].file = tempBundleRef;
					//GetIndString( ThePlug[tPlug].MenuName, 1000, 1);
					//ThePlug[tPlug].Type = 'PPDG';
					
					GetIndString( tStr, 1000, 1);
					BlockMoveData( tStr + 1, &ThePPINPlug[tPlug].type, 4);
					MOT32(&ThePPINPlug[tPlug].type);
					
					GetIndString( tStr, 1000, 2);
					BlockMoveData( tStr + 1, &ThePPINPlug[tPlug].mode, 4);
					MOT32(&ThePPINPlug[tPlug].mode);
					
					GetIndString( ThePPINPlug[tPlug].MenuName, 1000, 3);
					GetIndString( ThePPINPlug[tPlug].AuthorString, 1000, 4);
					
					GetIndString( tStr, 1000, 5);
					BlockMoveData( tStr + 1, &ThePPINPlug[tPlug].InsSamp, 4);
					MOT32(&ThePPINPlug[tPlug].InsSamp);
...
#endif
#pragma mark Universal Functions
...
OSErr PPINAvailablePlug( OSType kindFile, OSType *plugType)
{
	...
	for( i = 0; i < tPlug; i++)
	{
		...
		{
			if( plugType != NULL) *plugType = ThePPINPlug[ i].InsSamp;
			return noErr;
		}
	}
