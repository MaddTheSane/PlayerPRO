...
void SetUpScrollDigi(void)
{
	...
	SetCtlMin( ControlDigi, 0);
	SetCtlMax( ControlDigi, MAXTRACK - (viewRect.right - viewRect.left) / TEXTLARG);
...
{ 
		...
		GetPortVisibleRegion( GetDialogPort( DigitalDlog), visibleRegion);
 		...
 		UpdtDialog( DigitalDlog, visibleRegion);
		...
		for( x = GetCtlValue( ControlDigi); x < MAXTRACK; x++)
		{
...
void CreateDigiListWindow(void)
{
	...
	DigitalDlog = GetNewDialog( 150, 0L, GetDialogWindow( ToolsDlog));
