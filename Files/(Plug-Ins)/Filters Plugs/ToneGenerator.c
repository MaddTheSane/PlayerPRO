...
static short* CreateAudio16Ptr(long AudioLength, long AudioFreq, long AudioAmp, long AudioType, Boolean stereo)
{
	...
	switch (AudioType) {
		...
		case silence:
			memset(Audio16Ptr, 0, AudioLength * 2);
		...
		case triangle:
			...
			for (i = 0, x = 0, dest = -1; i < AudioLength; i++) {
				...
				if (UpDown)
					temp = (65535L * (dest - i)) / inter;
				else
					temp = (65535L * (inter - (dest - i))) / inter;
							   ...
							   short		StereoMode) // StereoMode = 0 apply on all channels, = 1 apply on current channel
{
	...
	if (AudioLength <= 0)
		AudioLength = 2000;
	NumToString(AudioLength, tStr);
	SetDText(myDia, 8, tStr);
	
	AudioFreq	= 440;
	NumToString(AudioFreq, tStr);
	SetDText(myDia, 9, tStr);
	AudioAmp	= 100;
	NumToString(AudioAmp, tStr);
	SetDText(myDia, 10, tStr);
	...
	do {
		...
		switch (itemHit) {
			...
			case 7:
			{
				...
				GetDText(myDia, 8, tStr);
				StringToNum(tStr, &AudioLength);
				GetDText(myDia, 9, tStr);
				StringToNum(tStr, &AudioFreq);
				GetDText(myDia, 10, tStr);
				StringToNum(tStr, &AudioAmp);
				
				switch (theData->amp) {
					case 8:
						if (Audio8Ptr != NULL) {
							DisposePtr(Audio8Ptr);
							Audio8Ptr = NULL;
						}
	...
	if (itemHit == 1) {
		...
		if (theData->amp == 8)
			BlockMoveData(Audio8Ptr, resultPtr + SelectionStart, AudioLength);
		else
			BlockMoveData(Audio16Ptr, resultPtr + SelectionStart, AudioLength);
