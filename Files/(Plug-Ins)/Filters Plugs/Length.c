...
ReGo:
	...
	do {
		...
		switch (itemHit) {
			...
			case 16:
				...
				GetDialogItem(myDia, 15, &itemType, &itemHandle, &itemRect);
				SetControlValue((ControlHandle) itemHandle, false);
				GetDialogItem(myDia, 16, &itemType, &itemHandle, &itemRect);
				SetControlValue((ControlHandle) itemHandle, false);
				...
				if (changeMode == 2)
					ControlSwitch(13, myDia, 0);
				else
					ControlSwitch(13, myDia, true);
				break;
	...
	if (itemHit == 1) {
		...
		GetDText(myDia, 7, tStr);
		StringToNum(tStr, &newSize);
		
		if (newSize <= 0 || newSize > 1024 * 1024 * 20) {
			SysBeep(1);
			SelectDialogItemText(myDia, 7, 0, 10000);
			goto ReGo;
		}
		...
		if (changeMode == 2) {
			...
			if (ChangeRate)
				theData->c2spd = (theData->c2spd * (newSize/100)) / (theData->size/100);
			...
			if (dst != NULL) {
				if (newSize > theData->size) {
					if (changeMode == 0)
						memcpy(dst, theData->data, theData->size);
					else {
					...
					if (changeMode == 0)
						memcpy(dst, theData->data, newSize);
					else {
		...
		if (theData->loopBeg > newSize) {
			theData->loopBeg = 0;
			theData->loopSize = 0;
		}
		if (theData->loopSize < 0)
			theData->loopSize = 0;
		if (theData->loopBeg + theData->loopSize > newSize)
			theData->loopSize = newSize - theData->loopBeg;
	}
