...
void ApplyFilter(Boolean filter, sData *SDataSrc)
{
	...
	if (logMode) {
		...
		if (filter)
			MADCallFFT(SDataSrc, ShiftFilter, MADDriver, filter);
		else
			MADCallFFT(SDataSrc, HzFilter, MADDriver, filter);
	}
}

void FFTSampleFilter(sData *SDataSrc, short instru, Boolean filter)
{
	short		itemHit, itemType, prev;
	int			i;
	Rect		itemRect;
	...
	TheDia = GetNewDialog(184, NULL, (WindowPtr) -1L);
	SetPortDialogPort(TheDia);
	AutoPosition(TheDia);
	ChangeDialogFont(TheDia);
	
	if (filter)
		SetWTitle(GetDialogWindow(TheDia), "\pShift Filter");
	else
		SetWTitle(GetDialogWindow(TheDia), "\pHz Filter");
	
	if (!logMode)
		TurnRadio(7, TheDia, true);
	else
		TurnRadio(8, TheDia, true);
		...
		switch (itemHit) {
			...
			case 3:
				...
				do {
					...
					if (oldValue < 0)
						oldValue = 0;
					if (oldValue >= EQPACKET/2)
						oldValue = EQPACKET/2 -1;
					
					if (NewValue < 0)
						NewValue = 0;
					if (NewValue >= EQPACKET / 2)
						NewValue = EQPACKET / 2 -1;
			...
			case 4:	// TEST
				...
				MADPlaySoundDataSYNC(MADDriver, SDataSrc->data, SData.size, 0, 48 + SData.relNote, SData.amp, SData.loopBeg, SData.loopSize, ((long)SData.c2spd) << 16, SData.stereo);
