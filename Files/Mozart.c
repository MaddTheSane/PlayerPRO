...
Rect	caRect;
	...
	ret = 1 + GetCtlValue( c3h) + (caRect.bottom - MozartRect.top) / YSize;
...
Rect	caRect;
	...
	r->top = MozartRect.top + (t - GetCtlValue( c3h))*YSize;
...
short PosToNote3( Point aPt, short	*yAxe, short *track)
{
	...
	*track += GetCtlValue( c3h);
	...
	xTr = GetCtlValue( c2h);	yTr = GetCtlValue( c1h);
	...
	if( yAxe != 0L)
	{
		*yAxe = aPt.v - MozartRect.top + yTr - (*track - GetCtlValue( c3h))*YSize;
		*yAxe /= MOHaut;
...
Point NoteToPos2( Point aNote, short track)
{
	...
	pos.v = aNote.v*MOHaut - yTr  + MozartRect.top + (track - GetCtlValue( c3h))*YSize;
...
void GetSelectionRect2( Rect *selecRect, Boolean ViewOnly)
{
	...
	if( ViewOnly)
	{
		...
		viewRect = MozartRect;
		viewRect.top = MozartRect.top + (selecTrack - GetCtlValue( c3h))*YSize;
...
void MozartHelp(void)
{
	...
	TheDia = GetNewDialog( 138,0L, (WindowPtr) -1L);
...
short TrackMarquee(Point start, Rect *resultRect, Rect *maxRect)
{
	...
	do
	{
		...
		if( !EqualRect( &nowRect, &thenRect))
		{
			...
			starto	= PosToNote4( topLeft( thenRect), 0L, track);
			endo	= PosToNote4( topLeft( nowRect), 0L, track);
			
			if( starto == endo)
			{
				starto	= PosToNote4( botRight( thenRect), 0L, track);
				endo	= PosToNote4( botRight( nowRect), 0L, track);
			}
...
Str255		mainStr, stemp;
	...
	for( i = 0; i < x; i++) DelMenuItem( InstruMenu, 1);
...
void ComputeGrilleGWorld()
{
	...
	GetMozartTrackRect( &r, GetCtlValue( c3h));
	r.right = caRect.right-15;
//	r.bottom = MozartDlog->portRect.bottom-15;
	...
	EraseRect( &pixBounds);
	TraceGrille2( -GetCtlValue( c2h), -GetCtlValue( c1h), &pixBounds);
...
/*	for( tI = GetCtlValue( c3h); tI < curMusic->header->numChn; tI++)
	...
	for( tI = GetCtlValue( c3h); tI < GetMaxYBox(); tI++)	//
	{
		Str255		String;
				
		/* Le ClipRect */
		theClipRect.left = pixMapRect.left;
		theClipRect.top = MozartRect.top + (tI - GetCtlValue( c3h))*YSize;
		...
		destRect.top	= MozartRect.top + 1 + (tI - GetCtlValue( c3h)) * YSize;
		...
		destRect.top -= yTr - (tI - GetCtlValue( c3h))*YSize;
		destRect.bottom -= yTr - (tI - GetCtlValue( c3h))*YSize;
...
void DrawPartitionReader()
{
	...
	tRect.left = MozartRect.left + ReaderCopy * MOLarg - GetCtlValue( c2h);
...
{ 
	...
	BeginUpdate( GetDialogWindow( GetSelection));
		...
		GetMozartTrackRect( &tempRect, GetCtlValue( c3h));
	...
	if( copyval != curVal)
	{
		...
		else	// Control c1h
		{
			...
			for( tI = GetCtlValue( c3h); tI < curMusic->header->numChn; tI++)
			{
				...
				itemRect.top += 1 + (tI - GetCtlValue( c3h))*YSize;
				itemRect.right = caRect.right - 15;
...
void GetNoteRect2( Rect *destRect, Point theCell, short track)
{
	...
	destRect->top = MozartRect.top + theCell.v * MOHaut - yTr + (track - GetCtlValue( c3h))*YSize;
...
void PressPartition( Point myPt)
{
	...
	switch( Mode)
	{
		case ZoomM:
			...
			GetShowVal( &XS, &XE, &YS, &YE);
			SetCtlValue( c2h, (theCell.h - (XE - XS)/2) * MOLarg);
			SetCtlValue( c1h, (theCell.v - (YE - YS)/2) * MOHaut);
...
void DoItemPressMozart( short whichItem, DialogPtr whichDialog)
{
		Point				myPt, theCell, PointNote, PointNote2, XxX, Note, dest;
 		short				ctlPart, bogus, track, xTr, yTr, i, itemType, oldPosM, xVal, yVal, Pos, val;
 		ControlHandle		theControl;
 		GrafPtr				SavePort;
		...
		//	ctlPart = FindControl( myPt, GetDialogWindow( whichDialog), &theControl);
			...
			if( GetCRefCon( theControl) == 1 || theControl == c2h || theControl == c3h)
			{
		...
		Rect	caRect;
			...
			case HelpM:
				if( MyTrackControl( HelpBut, theEvent.where, 0L))
				{
			...
			case PrefM:
				if( MyTrackControl( prefBut, theEvent.where, 0L))
				{
					#include "Help.h"
...
void CreateMozartWindow(void)
{
	...
	MozartDlog = GetNewDialog( 135, 0L, GetDialogWindow( ToolsDlog));
	SetWindEtat( GetDialogWindow( MozartDlog));
...
/*	GetDialogItem( MozartDlog , SeeAllM, &itemType, &itemHandle, &itemRect);
	...
	if( !GetIns( &curInstru, 0L)) curInstru = 0;
	curInstru++;
	...
	SetCtlValue( c1h, thePrefs.MozartC1h);
	SetCtlValue( c2h, thePrefs.MozartC2h);
	...
	if( DragManagerUse)
	{
		...
		InstallTrackingHandler( (DragTrackingHandlerUPP) MyTrackingHandlerUPP, GetDialogWindow( MozartDlog), (void *) 0L);
		InstallReceiveHandler( (DragReceiveHandlerUPP) MyReceiveDropHandlerUPP, GetDialogWindow( MozartDlog), (void *) 0L);
	}
}
...
void CloseMozartWindow(void)
{
	if( MozartDlog != 0L)
	{
		thePrefs.MozartC1h = GetCtlValue( c1h);
		thePrefs.MozartC2h = GetCtlValue( c2h);
...
Rect	caRect;
	...
	SetRect(	&yRect,
				...
				MozartRect.top + (DropY*MOHaut - GetCtlValue( c1h)) + (track - GetCtlValue( c3h))*YSize,
				MozartRect.left,
				MozartRect.top + (DropY*MOHaut - GetCtlValue( c1h)) + (track - GetCtlValue( c3h))*YSize + MOHaut*(selecEnd.v - selecStart.v + 1) + 1);
	if( yRect.bottom > caRect.bottom -15)
		yRect.bottom = caRect.bottom -15;
	
	SetRect( 	&xRect,
				MozartRect.left + (DropX*MOLarg - GetCtlValue( c2h)),
				MozartRect.top - LegendeHi + 1,
				MozartRect.left + (DropX*MOLarg - GetCtlValue( c2h)) + MOLarg*(selecEnd.h - selecStart.h + 1) + 1,
				MozartRect.top );
	...
	SetRect( &maxRectY, MozartRect.left-LegendeLa, MozartRect.top + 1 + (track - GetCtlValue( c3h))*YSize, MozartRect.left, MozartRect.top + 1 + (track - GetCtlValue( c3h))*YSize + YSize);
	...
	switch (message)
	{
		...
		case kDragTrackingInWindow:
			...
			if (attributes & kDragInsideSenderWindow)
			{
				...
				if( PtInRect( localMouse, GetControlBounds( PoubBut, &bounds)) && selectedControl == 0L)	{ HiliteControl( PoubBut, kControlButtonPart);		selectedControl = PoubBut;}
			...
			if( PtInRect( localMouse, &MozartRect))
			{
				...
				tempRect.left = trackRect.left + (pos * MOLarg - GetCtlValue( c2h));
				tempRect.top = trackRect.top + (note*MOHaut - GetCtlValue( c1h));
...
pascal OSErr MyReceiveDropBox(WindowPtr theWindow, void *handlerRefCon, DragReference theDrag)
{
	...
	if( DropX == -1 && DropY == -1 && DropTrack == -1 && selectedControl == 0L) return( dragNotAcceptedErr);
