...
void DrawQTItem( Boolean selected, Rect* bounds, Str255 data, FSSpec	*spec, RgnHandle rgn)
{
	...
	#if MACOS9VERSION
	iErr = GetFileIcon( spec, svAllLargeData, &gTheSuite);
	if( iErr == noErr)
	{
		if( !selected) PlotIconSuite(&iconRect, atNone, ttNone, gTheSuite);
		else PlotIconSuite(&iconRect, atNone, ttSelected, gTheSuite);

		if( rgn)
		{
			IconSuiteToRgn( rgn, bounds, 0, gTheSuite);
		}
	...
	{
		if( !selected) PlotIconID(&iconRect, atNone, ttNone, 188);
		else PlotIconID(&iconRect, atNone, ttSelected, 188);
	}
	#else
...
/*	{
	...
	if( lastSeconds != aTime / currentTime.scale)
	{
		...
		Secs2Date( aTime / currentTime.scale, &dtrp);
...
void SelectQTFile( FSSpec	*file)
{
	...
	if( QTMovie != 0L)
	{
		MyDebugStr(__LINE__, __FILE__, "QTMovie != 0L");
	}
	...
	resId = 0;
	iErr = NewMovieFromFile( &QTMovie, QTresRefNum, &resId, 0L, 0, &dataRefWasChanged);
...
/*	{
	...
	for( i = 1; i <= GetMovieTrackCount( QTMovie); i++)
	{
		...
		if( mediaType == SoundMediaType)	// Bingo !
		{
			...
			Secs2Date( aTime / aTimeScale, &dtrp);
...
Boolean CreateQTWindow( FSSpec	*file)
{
	...
	QuicktimeDlog = GetNewDialog( 180, 0L, (WindowPtr) ToolsDlog);
	...
	if( QTMovie == 0L)	// Quicktime wasn't able to open it....
	{
		CloseQT();
	...
	if( DragManagerUse)
	{
		...
		InstallTrackingHandler( (DragTrackingHandlerUPP) MyTrackingQTUPP, GetDialogWindow( QuicktimeDlog), (void *) 0L);
		InstallReceiveHandler( (DragReceiveHandlerUPP) MyReceiveQTUPP, GetDialogWindow( QuicktimeDlog), (void *) 0L);
	}
...
void DoItemPressQT( short whichItem, DialogPtr whichDialog)
{
		...
		if( !MCIsPlayerEvent( gMovieController, &theEvent))
		{
			switch( whichItem)
			{
				...
				case 25:
					if( GetControlHilite( ResetBassBut)  == 0 && MyTrackControl( ResetBassBut, theEvent.where, 0L))
					{
						HiliteControl( ResetBassBut, 255);
				...
				case 24:
					if( GetControlHilite( ResetTrebleBut) == 0 && MyTrackControl( ResetTrebleBut, theEvent.where, 0L))
					{
						HiliteControl( ResetTrebleBut, 255);
				...
				case 21:
					if( GetControlHilite( ResetBalanceBut) == 0 && MyTrackControl( ResetBalanceBut, theEvent.where, 0L))
					{
						HiliteControl( ResetBalanceBut, 255);
...
void CloseQT(void)
{
	...
	if( QuicktimeDlog != 0L)
	{
		...
		*/
		...
		if( MyTrackingQTUPP != 0L && MyReceiveQTUPP != 0L)
		{
