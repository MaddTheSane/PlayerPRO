...
void DrawMODListItem( short iD)
{
	...
	tempRect.top = myList.rect.top + myList.HCell* (iD - GetCtlValue( myList.yScroll));
...
pascal short MyOMatchNextAlphabetically( Ptr cellDataPtr, Ptr searchDataPtr, short cellDataLen, short searchDataLen)
{
	if( cellDataLen > 0)
	{
		if( IdenticalText( cellDataPtr, searchDataPtr, searchDataLen, searchDataLen, 0L) == 0)
		{
			return false;
		}
	}
...
BitMap		screenBits;
	...
	if( caRect.right == BIGMUSICLIST) SetCtlValue( FlipBut, 0);
...
UseResFile( iFileRefI);
	...
	for( theCell.v = val, i = 1; theCell.v < theNo + val ; theCell.v++, i += 2)
	{
		...
		if( iErr == noErr) HGetVol( 0L, &myFSS.vRefNum, &myFSS.parID);
	...
	switch (message) {
		...
		case kDragTrackingInWindow:
			...
			if (attributes & kDragInsideSenderWindow)
			{
				...
				if( PtInRect( localMouse, &contrlRect) && selectedControl == 0L) { HiliteControl( InfoBut, kControlButtonPart);		selectedControl = InfoBut;}
			...
			if( PtInRect( localMouse, &myList.rect))
			{
				...
				DropZone += GetCtlValue( myList.yScroll);
				...
				else
				{
					tempRect.left = myList.rect.left;
					tempRect.right = myList.rect.right;
					tempRect.top = tempRect.bottom = ((DropZone - GetCtlValue( myList.yScroll)) * myList.HCell) + myList.rect.top;
	...
	//	SetCursor( &CHandCrsr);
		if( WaitMouseMoved( theEvent.where))
		{
			...
			for( i = 0; i < myList.select.bottom - myList.select.top + 1; i++)
			{
				...
				#if MACOS9VERSION
				if( GetFileIcon( &myFSS, svAllSmallData, &gTheSuite) == noErr)
				{
					IconSuiteToRgn( tempRgn, &cellRect, 0,gTheSuite);
					
					DisposeIconSuite( gTheSuite, true);
				}
				else
				{
					IconIDToRgn( tempRgn, &cellRect, 0, 133);
				}
				#else
				IconRef	iconref;
				SInt16	label;
...
//	mySearchUPP2 = NewListSearchProc( MyOMatchNextList);
	...
	MODListDlog = GetNewDialog( 142, 0L, (WindowPtr) ToolsDlog);
...
//	itemRect.right = itemRect.left + 32;
	...
	if( DragManagerUse)
	{
		...
		InstallTrackingHandler( (DragTrackingHandlerUPP) MyTrackingMODListUPP, GetDialogWindow( MODListDlog), (void *) 0L);
		InstallReceiveHandler( (DragReceiveHandlerUPP) MyReceiveMODListUPP, GetDialogWindow( MODListDlog), (void *) 0L);
	}
...
Rect			caRect;
	...
	//	iErr = HSetVol( 0L, VRef, parID);
		...
		if( iErr == -43 || iErr == -35)
		{
			...
			iErr = PBHSetVol( &wdpb, false);
			if( iErr == noErr) HGetVol( 0L, &VRef, &parID);
			else {VRef	= 0;	parID	= 0;}
		}
		...
		OSType2Ptr( fndrInfo.fdType, tempC);
	 	if( OpenableFile( fndrInfo.fdType, &fss) == false && MADMusicIdentifyFSp( gMADLib, tempC, &fss) != noErr)
	 	{
	 		...
	 		if( fileSize > 1000)
			{
				...
				pStrcat( StrTemp, "\p Mb");
			}
			else
			{
				NumToString( fileSize, StrTemp);
				pStrcat( StrTemp, "\p Kb");
...
GrafPtr		SavePort;
	...
	if( MODListDlog == 0L) return noErr;
...
void ShowCopyrightNote()
{
	...
	TheDia = GetNewDialog( 177,0L, (WindowPtr) -1L);
...
void DoChangeMODName(void)
{
	...
	TheDia = GetNewDialog( 148,0L, (WindowPtr) -1L);
	SetPortDialogPort( TheDia);
	AutoPosition( TheDia);
...
/*	if( curMusic->header->MODMode) TurnRadio( 18, TheDia, true);
	...
	if( itemHit == 1)
	{
		...
		if( GetCtlValue( (ControlHandle) itemHandle)) curMusic->header->showCopyright = true;
		else curMusic->header->showCopyright = false;
		
		GetDialogItem(TheDia, 18, &itemType, &itemHandle, &itemRect);
		if( GetCtlValue( (ControlHandle) itemHandle)) curMusic->header->MODMode = true;
		else curMusic->header->MODMode = false;
		
		GetDialogItem(TheDia, 20, &itemType, &itemHandle, &itemRect);
		if( GetCtlValue( (ControlHandle) itemHandle)) curMusic->header->MultiChan = true;
		else curMusic->header->MultiChan = false;
		
		GetDialogItem(TheDia, 19, &itemType, &itemHandle, &itemRect);
		if( GetCtlValue( (ControlHandle) itemHandle)) curMusic->header->XMLinear = true;
...
OSErr	iErr;
	...
	BeginUpdate( GetDialogWindow( theDia));
		...
		#if MACOS9VERSION
		if( GetFileIcon( specList[ FileInfoID], svAllAvailableData, &gTheSuite) == noErr)
		{
			GetDialogItem( theDia, 11, &itemType, &itemHandle, &itemRect);
			
			EraseRect( &itemRect);
			PlotIconSuite( &itemRect, atNone, ttNone, gTheSuite);
...
void FileInformations( short whichItem)
{
	...
	*((OSType*)(aStr+1)) = fndrInfo.fdCreator;
	...
	else
	{
		...
		iErr = HSetVol( 0L, myFSS.vRefNum, myFSS.parID);
...
OSErr CheckFileAvailable( short ID)
{
	...
	if( iErr == -43 || iErr == -35)
	{
		...
		wdpb.ioNamePtr = pathStrList[ ID];
		iErr = PBHSetVol( &wdpb, false);
		if( iErr == noErr) HGetVol( 0L, &specList[ ID]->vRefNum, &specList[ ID]->parID);
	...
	if( iErr == -43 || iErr == -35)
	{
		...
		ParamText( specList[ ID]->name, 0L, 0L, 0L);
		
		aDia = GetNewDialog( 3003,0L, (WindowPtr) -1L);
...
*/
	...
	ParamText( curMusicList.name, 0L, 0L, 0L);
	
	aDia = GetNewDialog( 3004,0L, (WindowPtr) -1L);
	SetPortDialogPort( aDia);
		...
		if( PLGetSelect( &theCell, &myList))
		{
			if( CheckFileAvailable( theCell.v) == noErr)
			{
				...
				if( file != 0L)
				{
					...
					ZeroScrap();
					PutScrap( inOutCount, fndrInfo.fdType, file);
