...
{ 
		...
		GetPortVisibleRegion( GetDialogPort( PartiDlog), visibleRegion);
 		...
 		UpdtDialog( PartiDlog, visibleRegion);
		...
		switch( whichItem)
		{
			...
			if( GetControlHilite( RemoveBut) == 0 && MyTrackControl( RemoveBut, theEvent.where, 0L))
			{
				...
				if( PLGetSelect( &theCell, &myList))
				{
					...
					curMusic->header->numPointers--;
					if( curMusic->header->numPointers < 1) curMusic->header->numPointers = 1;
...
void CreatePartiWindow(void)
{
	...
	PartiDlog = GetNewDialog( 166, 0L, GetDialogWindow( ToolsDlog));
...
void COPYParti(void)
{
	...
	if( PLGetSelect( &theCell, &myList))
	{
		...
		ZeroScrap();
		PutScrap( (long) GetHandleSize( theHandle), 'PATL', (Ptr) *theHandle);
...
OSErr				anErr;
	...
	lCntOrErr = GetScrap( 0L, 'PATL', &scrapOffset);
	...
	if( lCntOrErr > 0)
	{
		...
		#if MACOS9VERSION
		GetScrap( theHandle, 'PATL', &scrapOffset);
		#else
		HLock( theHandle);
