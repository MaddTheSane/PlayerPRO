...
double					val;
	...
	aDia = GetNewDialog( 182, 0L, (WindowPtr) -1L);
...
return;
	...
	for( i = 0; i < curMusic->header->numChn; i++)
	{
		if( MADDriver->Tube[i] >= -MINPULSE*2-1)
		{
			...
			{
				if( MADDriver->chan[ i].maxPtr > MADDriver->chan[ i].curPtr && MADDriver->chan[ i].curPtr != 0L && MADDriver->Tube[i] <= 0)
			 	{
...
//	if( oldWindow == AdapDlog)
	{
		...
		if( PtInRgn( pt, visibleRegion))
		{
			...
			if( found)
			{
				...
				else DrawValueIndicator( val, percent, 0L);
			}
			else DrawValueIndicator( -1, false, 0L);
		}
		else DrawValueIndicator( -1, false, 0L);
...
Rect	caRect;
	...
	SetCtlMin( ScrollAdap, 0);
	
	if( MADDriver->DriverSettings.outPutMode == DeluxeStereoOutPut)
	{
		SetCtlMax( ScrollAdap, gCurrentTrack - (viewRect.bottom - viewRect.top) / DISVOL);
	}
	else SetCtlMax( ScrollAdap, gCurrentTrack - (viewRect.bottom - viewRect.top) / DISVOL);
	
	if( GetCtlMax( ScrollAdap) < 0) SetCtlMax( ScrollAdap, 0);
...
void FillOnOffControl( short id, short curval)
{
	...
	if( curval)
	{
		...
		SetCtlValue( OnOff[ id], 1);
	}
	else
	{
		HiliteControl( volCntl[ id], 255);
		HiliteControl( pannCntl[ id], 255);
		SetCtlValue( OnOff[ id], 0);
	}
	
	GetControlBounds( OnOff[ id], &contrlRect);
	
	MyMoveControl( OnOff[ id], contrlRect.left, VolumeRect.top - (GetCtlValue( ScrollAdap) - id)*DISVOL);
...
void FillVSTOnOffControl( short id, short curval)
{
	...
	if( curval)
	{
		SetCtlValue( VSTOnOff[ id], 1);
	}
	else
	{
		SetCtlValue( VSTOnOff[ id], 0);
	}
	
	GetControlBounds( VSTOnOff[ id], &contrlRect);
	
	MyMoveControl( VSTOnOff[ id], contrlRect.left, VolumeRect.top - (GetCtlValue( ScrollAdap) - id)*DISVOL);
...
void FillFxControl( short id)
{
	...
	for( x = 0; x < 4; x++)
	{
		...
		MyMoveControl( VSTFx[ id][ x], contrlRect.left, VolumeRect.top - (GetCtlValue( ScrollAdap) - id)*DISVOL);
		...
		if( curMusic->header->chanBus[ id].copyId == id || curMusic->header->chanBus[ id].Active == false)
		{
			...
			{
				if( MADDriver->chanVST[ id][ x] != 0L) HiliteControl( VSTFx[ id][ x], kControlButtonPart);
...
void FillFxDestControl( short id)
{
	...
	MyMoveControl( VSTDest[ id], contrlRect.left, VolumeRect.top - (GetCtlValue( ScrollAdap) - id)*DISVOL);
...
/*	if( curval)
	...
	MyMoveControl( Mono[ id], contrlRect.left, VolumeRect.top - (GetCtlValue( ScrollAdap) - id)*DISVOL);
...
RGBColor	color;
	...
	MyMoveControl( volCntl[ id], contrlRect.left, VolumeRect.top - (GetCtlValue( ScrollAdap) - id)*DISVOL);
...
Rect	contrlRect;
	...
	MyMoveControl( pannCntl[ id], contrlRect.left, VolumeRect.top - (GetCtlValue( ScrollAdap) - id)*DISVOL - 1);
...
Rect	itemRect;
	...
	pulseRect[ id].top = VolumeRect.top - (GetCtlValue( ScrollAdap) - id)*DISVOL;
...
void UpdateAdapWindow( DialogPtr	GetSelection)
{
		...
		DisposeRgn( visibleRegion);
 		...
 		if( GetCtlValue( ScrollAdap) + max > gCurrentTrack) max = gCurrentTrack - GetCtlValue( ScrollAdap);
		...
		for( i = 0; i < 10; i++)
		{
			...
			{
	 			if( MADDriver->masterVST[ i] != 0L) HiliteControl( MainFx[ i], kControlButtonPart);
		...
		SetCtlValue( MainFXOnOff, curMusic->header->globalFXActive);
...
//	ScrollAdap = (ControlHandle) itemHandle;
	...
	for( i = 0; i < MAXTRACK; i++)
	{
		for( x = 0; x < 4; x++)
		{
			if( MADDriver->chanVST[ i][ x] != 0L) HiliteControl( VSTFx[ i][ x], kControlButtonPart);
...
Handle	itemHandle;
	...
	for( i = 0; i < MAXTRACK; i++)
	{
		if( volCntl[ i]) { DisposeControl( volCntl[ i]);	volCntl[ i] = 0L;}
		if( Mono[ i]) { DisposeControl( Mono[ i]);	Mono[ i] = 0L;}
		if( pannCntl[ i]) { DisposeControl( pannCntl[ i]);	pannCntl[ i] = 0L;}
		
		if( VSTOnOff[ i]) { DisposeControl( VSTOnOff[ i]);	VSTOnOff[ i] = 0L;}
		if( VSTDest[ i]) { DisposeControl( VSTDest[ i]);	VSTDest[ i] = 0L;}
		if( OnOff[ i]) { DisposeControl( OnOff[ i]);	OnOff[ i] = 0L;}
		
		for( x = 0; x < 4; x++)
		{
			if( VSTFx[ i][ x]) { DisposeControl( VSTFx[ i][ x]);	VSTFx[ i][ x] = 0L;}
		}
	}
...
void InitAdapWindow(void)
{
	...
	AdapDlog = GetNewDialog( 141, 0L, GetDialogWindow( ToolsDlog));
...
/*	if( thePrefs.ADAPremember)
	...
	if( DragManagerUse)
	{
		...
		InstallTrackingHandler( (DragTrackingHandlerUPP) MyTrackingAdapUPP, GetDialogWindow( AdapDlog), (void *) 0L);
		InstallReceiveHandler( (DragReceiveHandlerUPP) MyReceiveAdapUPP, GetDialogWindow( AdapDlog), (void *) 0L);
	}
}
...
/*	case 18:
		...
		if(PtInRect( Location, &itemRect))
		{
    		...
    		//			QDFlushPortBuffer( GetDialogPort( whichDialog), NULL);
				...
				if( oldH != Location.h)
				{
					...
					DrawValueIndicator( 100. * (MADDriver->VExt / 8000.), true, 0L);
					...
					DrawValueIndicator( 100.*(8000./ (MADDriver->FreqExt)), true, 0L);
					}
				}
			}
		...
		if(PtInRect( Location, &itemRect))
		{
    		...
    		//			QDFlushPortBuffer( GetDialogPort( whichDialog), NULL);
				...
				if( oldH != Location.h)
				{
					...
					DrawValueIndicator( 100.*((MADDriver->globPan - 64)/64.), true, 0L);
	...
	case 41:
		if( GetControlHilite( FineBut) == 0  && MyTrackControl( FineBut, theEvent.where, 0L))
		{
	...
	case 44:
		...
		if( GetControlHilite( ResetPanBut) == 0  && MyTrackControl( ResetPanBut, theEvent.where, 0L))
		{
	...
	case 22:
		...
		if( GetControlHilite( ResetSpeedBut) == 0  && MyTrackControl( ResetSpeedBut, theEvent.where, 0L))
		{
	...
	case 23:
		...
		if( GetControlHilite( ResetPitchBut) == 0  && MyTrackControl( ResetPitchBut, theEvent.where, 0L))
		{
...
/*	case 23:
		...
		if(PtInRect( Location, &itemRect))
		{
    		...
    		//			QDFlushPortBuffer( GetDialogPort( whichDialog), NULL);
				...
				if( oldH != Location.h)
				{
					...
					DrawValueIndicator( (100*MADDriver->VolGlobal)/64, true, 0L);
				}
			}
	...
	case 32:
		...
		if( MyTrackControl( MainFXOnOff, theEvent.where, 0L))
		{
			Rect contrlRect;
	...
	case 16:	// Volume Voice
		...
		if( theControl)
		{
			...
			switch( kindCntl)
			{
							...
							switch( temp)
							{
								case 1:	// No Effect 
									if( MADDriver->chanVST[ i][ whichFxTrack])
									{
										...
										MADDriver->chanVST[ i][ whichFxTrack] = 0L;
									}
								break;
				...
				case 'M':
					if( GetControlHilite( Mono[ i]) == 0 && MyTrackControl( Mono[ i], theEvent.where, 0L))
					{
						if( IsPressed( 0x3A))
						{
							short		ww;
						...
						else*/
						{
    						...
    						//			QDFlushPortBuffer( GetDialogPort( whichDialog), NULL);
								...
								if( oldH != Location.h)
								{
									...
									DrawValueIndicator( (100L*curMusic->header->chanVol[ i])/64L, true, 0L);
								}
							}
						...
						if( GetControlHilite( Mono[ i]) == 0)
						{
							do
							{
								...
								if( oldH != Location.h)
								{
									...
									DrawValueIndicator( 100.*((curMusic->header->chanPan[ i] - 32) / 32.), true, 0L);
								}
								WaitNextEvent( everyEvent, &theEvent, 1, 0L);
				...
				case 'E':
					...
					if( GetControlHilite( VSTOnOff[ i]) == 0 && MyTrackControl( VSTOnOff[ i], theEvent.where, 0L))
					{
						if( IsPressed( 0x3A))
						{
							short		ww, noActive;
				...
				case 'O':
					if( GetControlHilite( OnOff[ i]) == 0 && MyTrackControl( OnOff[ i], theEvent.where, 0L))
					{
						if( IsPressed( 0x3A))
						{
							short		ww, noActive;
