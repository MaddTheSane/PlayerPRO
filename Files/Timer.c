...
1000) and a precision of better than 200 탎. Under System 6 the precision will
...
long StopTimer(Timer *t);					// 탎, up to 36 minutes
double StopTimerSecs(Timer *t);				// s, 탎 precision and no time limit
	...
	t=StopTimer(timer);
	DisposeTimer(timer);
//	printf("One hundred iterations takes %ld 탎\n",t);

The timing result comes in two flavors. StopTimer() returns the time in
microseconds as a long, which can hold a time up to 2,147,483,647 탎, which is
...
StopTimer() adds a small offset (about 62 탎 on a Mac IIci) to the raw time in
...
provided in 탎 in your Timer structure in the long structure member
"timeToStartTimer".

	betweenTime = StopTimer(t) - t->timeToStartTimer;
	
If you use StopTimerSecs() instead you'll have to divide timeToStartTimer by a
million to convert 탎 to s. For some purposes this offset is negligibly
small. On my Mac IIci the timeToStartTimer is 240 탎. This offset, and the one
...
struct Timer{
	...
	long timeToStartTimer;			// minimum time in 탎
	long stopDelay;					// 탎 from call to stop, re from call to start
...
long StopTimer(Timer *t)							// Returns 탎
{
	...
	if(microSeconds>0) microSeconds*=1000;			// convert ms to 탎
	else microSeconds=-microSeconds;
	if(t->time.tmCount>0)t->time.tmCount*=-1000;	// convert ms to -탎
	microSeconds+=t->time.tmCount;					// -탎 until end of interval
...
double StopTimerSecs(Timer *t)						// Returns secs
{
	...
	else s=(double)-t->elapsedIntervals*t->interval;				// 탎
	if(t->time.tmCount>0) s-=t->time.tmCount*1000.0;	// -ms until end of interval
	else s+=t->time.tmCount;							// -탎 until end of interval
...
#if defined(powerc) || defined (__powerc)
static pascal void TimerTask(Timer *t)			// Called at interrupt time
{
