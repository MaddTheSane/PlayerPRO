...
void ChangeMapping( ICInstance inst, Handle prefh, long pos, OSType type, unsigned char* extension, ICMapEntry *entry, Boolean add)
{
	...
	entry->file_creator = 'SNPL';
	entry->post_creator = 'SNPL';
	
	pStrcpy( entry->extension, extension);
	pStrcpy( entry->MIME_type, "\paudio/x-");
	
	entry->MIME_type[ 9] = extension[ 2];
	entry->MIME_type[10] = extension[ 3];
	if( extension[ 0] > 2) entry->MIME_type[11] = extension[ 4];
	if( extension[ 0] > 3) entry->MIME_type[12] = extension[ 5];
	
	entry->MIME_type[ 0] += extension[ 0];
	
	pStrcpy( entry->creator_app_name, "\pPlayerPRO");
	pStrcpy( entry->post_app_name, "\pPlayerPRO");
	pStrcpy( entry->entry_name, "\pPlayerPRO");
                ...
                for( i = 0; i < count; i++)
                {
                	 ...
                	 switch( entry.file_type)
                	 {
                	 	case 'MADK':
                	 		madk = true;
                	 		mad = true;
                	 		ChangeMapping( inst, prefh, pos, entry.file_type, "\p.madk", &entry, false);
                	 	break;
                	 	
                	 	case 'MADI':
                	 		madi = true;
                	 		mad = true;
                	 		ChangeMapping( inst, prefh, pos, entry.file_type, "\p.madi", &entry, false);
                	 	break;
                	 	
                	 	case 'MADH':
                	 		madh = true;
                	 		ChangeMapping( inst, prefh, pos, entry.file_type, "\p.madh", &entry, false);
                	 	break;
                	 	...
                	 	case 'XM  ':
                	 		xm = true;
                	 		ChangeMapping( inst, prefh, pos, entry.file_type, "\p.xm", &entry, false);
                	 	break;
                	 	
                	 	case 'STrk':
                	 		mod = true;
                	 		ChangeMapping( inst, prefh, pos, entry.file_type, "\p.mod", &entry, false);
                	 	break;
                	 	
                	 	case 'S3M ':
                	 		s3m = true;
                	 		ChangeMapping( inst, prefh, pos, entry.file_type, "\p.s3m", &entry, false);
                	 	break;
                	 	
                	 	case 'IT  ':
                	 		it = true;
                	 		ChangeMapping( inst, prefh, pos, entry.file_type, "\p.it", &entry, false);
                	 	break;
