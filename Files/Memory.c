...
void DoNullMemWindow(void)
{
	...
	if( tAlpha != FreeMemGlobalK)
	{
		...
		UsedMem = (long) GetApplLimit() - (long) GetZone();
		...
		//***** Driver ********
		tempLong = 0;
	//	if( MADDriver->Vol != 0L) 			tempLong += GetPtrSize( MADDriver->Vol);
		if( MADDriver->IntDataPtr != 0L) 	tempLong += GetPtrSize( MADDriver->IntDataPtr);
	...
	/*	for( i = 0; i < MAX_PITCH; i++)
		...
		for( i = 0; i < MAXINSTRU; i++)
		{
			...
			{
				if( curMusic->sample[ curMusic->fid[ i].firstSample + x] == 0L) MyDebugStr( __LINE__, __FILE__, "numSamples ERR");
...
{ 
		...
		GetPortVisibleRegion( GetDialogPort( MemoryDlog), visibleRegion);
 		...
 		UpdtDialog( MemoryDlog, visibleRegion);
...
void CreateMemWindow(void)
{
	...
	MemoryDlog = GetNewDialog( 163, 0L, GetDialogWindow( ToolsDlog));
...
void CloseMem(void)
{
	if( MemoryDlog != 0L) DisposDialog( MemoryDlog);
	MemoryDlog = 0L;
