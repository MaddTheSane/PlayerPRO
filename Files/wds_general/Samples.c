...
void UpdateDisplaySize(short InstruNo)
{
	...
	switch (curSample[InstruNo]) {
		case panningEnv:
			NumToString(curMusic->fid[InstruNo].pannSize, StrTemp);
			pStrcat(StrTemp, "\p Pts");
			SetDText(SampleDlog[InstruNo], 3, StrTemp);
			break;
			
		case volumeEnv:
			NumToString(curMusic->fid[InstruNo].volSize, StrTemp);
			pStrcat(StrTemp, "\p Pts");
			SetDText(SampleDlog[InstruNo], 3, StrTemp);
			break;
			
		default:
			GetSizeString(SampleDataS(InstruNo, curSample[InstruNo])->size, StrTemp, false);
			SetDText(SampleDlog[InstruNo], 3, StrTemp);
			break;
	}
}
	...
	switch (curSample[InstruNo]) {
		...
		default:
			...
			if (SampleDataD(InstruNo)->amp == 16)
				val /= 2;
			if (SampleDataD(InstruNo)->stereo == true)
				val /= 2;
			sprintf(dest, "%02lX", val);
			MyC2PStr(dest);
			pStrcat(StrTemp, (unsigned char*) dest);
...
long PosToByte(short Pos, short InstruNo)
{
	...
	switch (curSample[InstruNo]) {
		...
		default:
			...
			if (tempL < 0)
				tempL = 0;
			if (tempL >= SampleDataD(InstruNo)->size)
				tempL = SampleDataD(InstruNo)->size;
			break;
	}
	...
	switch (curSample[InstruNo]) {
		case panningEnv:
			if (Clignote[InstruNo]) {
				curMusic->fid[InstruNo].pannType &= ~EFLOOP;
				...
				if (i >= curIns->pannSize)
					i = curIns->pannSize - 1;
				curIns->pannBeg = i;
				
				for (i = 0; i < curIns->pannSize; i++) {
					if (FindByteEnd(InstruNo) <= curIns->pannEnv[i].pos)
						break;
				}
				i--;
				if (i < 0)
		...
		case volumeEnv:
			if (Clignote[InstruNo]) {
				curMusic->fid[InstruNo].volType &= ~EFLOOP;
				...
				for (i = 0; i < curIns->volSize; i++) {
					if (FindByteEnd(InstruNo) <= curIns->volEnv[i].pos)
						break;
				}
				i--;
				if (i < 0)
			...
			if (Clignote[InstruNo]) {
				...
				if (SampleDataD(InstruNo)->stereo) {
					if (SampleDataD(InstruNo)->amp == 8) {
						SampleDataD(InstruNo)->loopBeg 	/= 2;
						SampleDataD(InstruNo)->loopBeg 	*= 2;
						SampleDataD(InstruNo)->loopSize	/= 2;
						SampleDataD(InstruNo)->loopSize	*= 2;
					} else {
						SampleDataD(InstruNo)->loopBeg 	/= 4;
						SampleDataD(InstruNo)->loopBeg 	*= 4;
						SampleDataD(InstruNo)->loopSize	/= 4;
						SampleDataD(InstruNo)->loopSize	*= 4;
					}
				}
			}
...
void SetSustainPoint(short InstruNo)
{
	...
	switch (curSample[InstruNo]) {
		case panningEnv:
			if (Clignote[InstruNo]) {
				curMusic->fid[InstruNo].pannType &= ~EFSUSTAIN;
		...
		case volumeEnv:
			if (Clignote[InstruNo]) {
				curMusic->fid[InstruNo].volType &= ~EFSUSTAIN;
	...
	switch (curSample[CurWin]) {
		case panningEnv:
		{
			...
			if (lCntOrErr > 0) {
				...
				for (i = curIns->pannSize - 1; i >= 0 ; i--) {
					if (curIns->pannEnv[i].pos >= Start) {
						...
						if (i + pasteEnv.pannSize < 12) {
							...
							if (curIns->pannEnv[i + pasteEnv.pannSize].pos >= ENVSIZE)
								curIns->pannEnv[i + pasteEnv.pannSize].pos = ENVSIZE - 1;
				...
				if (SelecRect[CurWin].end > ENVSIZE)
		...
		case volumeEnv:
		{
			...
			if (lCntOrErr > 0) {
				...
				for (i = curIns->volSize-1; i >= 0 ; i--) {
					if (curIns->volEnv[ i].pos >= Start) {
						...
						if (i + pasteEnv.volSize < 12) {
							...
							if (curIns->volEnv[i + pasteEnv.volSize].pos >= ENVSIZE)
								curIns->volEnv[i + pasteEnv.volSize].pos = ENVSIZE - 1;
				...
				for (i = 0; i < pasteEnv.volSize; i++) {
					...
					if (curIns->volEnv[i + whichPt].pos >= ENVSIZE)
						curIns->volEnv[i + whichPt].pos = ENVSIZE - 1;
...
void DrawLoop(short ins)
{
	...
	if (curSample[ins] >= 0) {
		...
		if (endLine > SampleRect.right)
			endLine = SampleRect.right + 2;
		
		PenSize(2,2);
		
		if (SizeLoop > 2) {
			temp = ByteToPos(StartLoop, ins);
			if (temp < SampleRect.left)
				temp = SampleRect.left - 2;
...
void DrawPencil(DialogPtr theDia, short InstruNo)
{
	...
	GetMouse(&prevPt);
	if (prevPt.v < SampleRect.top)
		prevPt.v = SampleRect.top;
	else if (prevPt.v > SampleRect.bottom)
		prevPt.v = SampleRect.bottom;
	
	if (prevPt.h < SampleRect.left)
		prevPt.h = SampleRect.left;
	else if (prevPt.h > SampleRect.right)
		prevPt.h = SampleRect.right;
	...
	do {
		...
		GetMouse(&aPt);
		if (aPt.v < SampleRect.top) {
			aPt.v = SampleRect.top;
			InOutPt = true;
		} else if (aPt.v > SampleRect.bottom) {
			aPt.v = SampleRect.bottom;
			InOutPt = true;
		} else
			InOutPt = false;
		
		if (InOutPt != PreInOutPt)
			firstTime = true;
		
		/**/
		
		if (aPt.h < SampleRect.left)
			aPt.h = SampleRect.left;
		else if (aPt.h > SampleRect.right)
			aPt.h = SampleRect.right;
		...
		if (prevPt.h != aPt.h || prevPt.v != aPt.v || firstTime == true) {
			...
			switch (curSample[InstruNo]) {
						...
						if (editPtEnv != -1) {
							for (i = 0; i < curIns->pannSize; i++) {
								if (i == editPtEnv) {
									...
									if (curIns->pannBeg >= editPtEnv && curIns->pannBeg > 0)
										curIns->pannBeg--;
									if (curIns->pannEnd >= editPtEnv && curIns->pannEnd > 0)
										curIns->pannEnd--;
									if (curIns->pannSus >= editPtEnv && curIns->pannSus > 0)
										curIns->pannSus--;
								}
							}
							...
							for (i = 0; i < curIns->pannSize; i++) {
								if (tcoc >= curIns->pannEnv[ i].pos - 1 && tcoc <= curIns->pannEnv[i].pos + 1) {
									...
									if (curIns->pannSize < 12) {
										...
										if (curIns->pannBeg >= editPtEnv)
											curIns->pannBeg++;
										if (curIns->pannEnd >= editPtEnv)
											curIns->pannEnd++;
										if (curIns->pannSus >= editPtEnv)
											curIns->pannSus++;
						...
						if (editPtEnv != -1) {
							for (i = 0; i < curIns->volSize; i++) {
								if (i == editPtEnv) {
									...
									if (curIns->volBeg >= editPtEnv && curIns->volBeg > 0)
										curIns->volBeg--;
									if (curIns->volEnd >= editPtEnv && curIns->volEnd > 0)
										curIns->volEnd--;
									if (curIns->volSus >= editPtEnv && curIns->volSus > 0)
										curIns->volSus--;
								}
							}
							...
							for (i = 0; i < curIns->volSize; i++) {
								if (tcoc >= curIns->volEnv[i].pos - 1 && tcoc <= curIns->volEnv[i].pos + 1) {
									...
									if (curIns->volSize < 12) {
										...
										if (curIns->volBeg >= editPtEnv)
											curIns->volBeg++;
										if (curIns->volEnd >= editPtEnv)
											curIns->volEnd++;
										if (curIns->volSus >= editPtEnv)
											curIns->volSus++;
				...
				default:
					...
					if (BS < 0)
						BS = 0;
					if (BE > SampleDataD(InstruNo)->size)
						BE = SampleDataD(InstruNo)->size;
					
					if (SampleDataD(InstruNo)->amp == 16) {
						nVal16S = (((long)(aPt.v - SampleRect.top) * 65535) / (long)(SampleRect.bottom - SampleRect.top));
						nVal16E = (((long)(prevPt.v - SampleRect.top) * 65535) / (long)(SampleRect.bottom - SampleRect.top));
						...
						if (BE > SampleDataD(InstruNo)->size / 2)
						...
						if (SampleDataD(InstruNo)->stereo) {
							...
							for (x = BS; x < BE; x++) {
								val = nVal16S + ((long)(x - BS) * (long)(nVal16E - nVal16S) / (BE - BS));
						...
						nVal8S = (((aPt.v - SampleRect.top) * 255L) / (long) (SampleRect.bottom - SampleRect.top));
						nVal8E = (((prevPt.v - SampleRect.top) * 255L) / (long) (SampleRect.bottom - SampleRect.top));
						...
						if (SampleDataD(InstruNo)->stereo) {
							...
							for (x = BS; x < BE; x++) {
								val = nVal8S + ((x - BS) * (long)(nVal8E - nVal8S) / (BE - BS));
							...
							for (x = BS; x < BE; x++) {
								val = nVal8S + ((long)(x - BS) * (long)(nVal8E - nVal8S) / (BE - BS));
								theSample[x] = val - 0x80;
							}
						}
					}
...
void DrawPanningEnveloppe(short ins, DialogPtr theDia)
{
	...
	TextSize(0);
	pStrcpy(aStr, "\pRight");
	MoveTo(SampleRect.left + 2, SampleRect.top + 13);
	DrawString(aStr);
	pStrcpy(aStr, "\pLeft");
	MoveTo(SampleRect.left + 2, SampleRect.bottom - 4);
	DrawString(aStr);
	TextFont(4);
	TextSize(9);
...
void InitSampleWindow(void)
{
	...
	SampleDlog[0] = GetNewDialog(137, NULL, GetDialogWindow(ToolsDlog));
...
void SetControlH(short theInstru)
{
	...
	if (ZoomLevel[ theInstru] != 1) {
		...
		SetControlMinimum(xScroll[theInstru], 0);
		SetControlMaximum(xScroll[theInstru], 0);
		SetControlValue(xScroll[theInstru], 0);
	}
}
	...
	recheck:
		switch (curSample[InstruNo]) {
			case panningEnv:
				...
				if (curMusic->fid[InstruNo].pannType & EFSUSTAIN)
			...
			case volumeEnv:
				...
				if (curMusic->fid[InstruNo].volType & EFON)
					HiliteControl(EnvBut[InstruNo], kControlButtonPart);
				else
					HiliteControl(EnvBut[InstruNo], 0);
				
				if (curMusic->fid[InstruNo].volType & EFSUSTAIN)
					HiliteControl(SustainBut[InstruNo], kControlButtonPart);
				else
					HiliteControl(SustainBut[InstruNo], 0);
...
void NCreateSampleWindow(short ins, short samp)
{
	...
	if (SampleDlog[ins] == NULL) {
		SampleDlog[ins] = GetNewDialog(137, NULL, GetDialogWindow(ToolsDlog));
	...
	if (IsPressed(0x3A) == true) {
		for (i = MAXINSTRU; i >= 0 ; i--) {
			if (SampleDlog[i] != NULL) {
				...
				if (BitMapSample[i] != NULL) {
					DisposeGWorld(BitMapSample[i]);
					BitMapSample[i] = NULL;
				}
				DisposeDialog(SampleDlog[i]);
				SampleDlog[i] = NULL;
			}
		}
	} else {
...
void NAppelPlug(short InstruNo, short samp, short whichPlug)
{
	...
	if (curData->stereo) {
		if (IsPressed(0x38) && curData->stereo)
			StereoMode = 0;
		else
			StereoMode = 1;
	} else
...
void NAppelVSTPlug(short InstruNo, short samp, short whichPlug)
{
	...
	if (SampleDataD(InstruNo)->stereo) {
		if (IsPressed(0x38) && SampleDataD(InstruNo)->stereo)
			StereoMode = 0;
		else
			StereoMode = 1;
	} else
...
void MoveLoopSelection(short InstruNo)
{
	...
	if (byte < SampleDataD(InstruNo)->loopBeg + SampleDataD(InstruNo)->loopSize / 2)
		StartPoint = true;
	else
		StartPoint = false;
	...
	do {
		...
		if (prevPt.h != aPt.h || firstTime == true) {
			...
			if (StartPoint) {
				if (PosToByte(aPt.h, InstruNo) < SampleDataD(InstruNo)->loopBeg + SampleDataD(InstruNo)->loopSize)
					StartPoint = true;
				else
					StartPoint = false;
			} else {
				if (PosToByte(aPt.h, InstruNo) < SampleDataD(InstruNo)->loopBeg)
					StartPoint = true;
				else
					StartPoint = false;
			}
				...
				if (curVoice->ins == InstruNo && curVoice->samp == curSample[InstruNo]) {
					if (curVoice->begPtr == SampleDataD(InstruNo)->data && curVoice->curPtr != curVoice->maxPtr) {
						if (SampleDataD(InstruNo)->loopSize > 2) {
							curVoice->maxPtr = (Ptr)((size_t)SampleDataD(InstruNo)->data +
													 SampleDataD(InstruNo)->loopBeg +
													 SampleDataD(InstruNo)->loopSize);
						}
...
void DoItemPressSample(short whichItem, DialogPtr whichDialog)
{
		...
		if (PtInRect(myPt, &SampleRect)) {
			...
			if (IsPressed(0x003A)) {
				...
				if (ZoomLevel[CurWin] > 1) {
					...
					SetControlValue(xScroll[CurWin], GetControlValue(xScroll[CurWin]) / 2);
				...
				if (ZoomLevel[CurWin] < 1024) {
					...
					SetControlValue(xScroll[CurWin], GetControlValue(xScroll[CurWin]) * 2 + bogus);
				...
				if (TickCount() - doubleClick <= GetDblTime() && lastPosition.h == myPt.h && lastPosition.v == myPt.v) {
					...
					if (IsPressed(0x0038)) {
						if (PosToByte(myPt.h, InstruNo) < SelecRect[InstruNo].end)
							...
							beginPoint = SelecRect[InstruNo].start;
					} else {
						...
						if (tL != otherPoint) {
							...
							switch (curSample[InstruNo]) {
								...
								default:
									if (Pt2.h < 0) {
										if (SelecRect[InstruNo].start < 0)
											SelecRect[InstruNo].start = 0;	//xxx
									...
									if (Pt2.h > caRect.right) {
										if (SelecRect[InstruNo].end > SampleDataD(InstruNo)->size)
											SelecRect[InstruNo].end = SampleDataD(InstruNo)->size;
	...
	switch (whichItem) {
		...
		case 13:
			if(curSample[InstruNo] < 0 && MyTrackControl(EnvBut[ InstruNo], theEvent.where, NULL)) {
				switch (curSample[InstruNo]) {
					case panningEnv:
						if (curMusic->fid[InstruNo].pannType & EFON) {
							curMusic->fid[InstruNo].pannType &= ~EFON;
							HiliteControl(EnvBut[InstruNo], 0);
						} else {
							curMusic->fid[InstruNo].pannType |= EFON;
							HiliteControl(EnvBut[InstruNo], kControlButtonPart);
					...
					case volumeEnv:
						if (curMusic->fid[InstruNo].volType & EFON) {
							curMusic->fid[InstruNo].volType &= ~EFON;
							HiliteControl(EnvBut[InstruNo], 0);
						} else {
							curMusic->fid[InstruNo].volType |= EFON;
							HiliteControl(EnvBut[InstruNo], kControlButtonPart);
		...
		case 26:
			if (GetControlHilite(PencilBut[InstruNo]) != 255 && MyTrackControl(PencilBut[InstruNo], theEvent.where, NULL)) {
				...
				if (GetControlHilite(ZoomBut[InstruNo]) != 255)
					HiliteControl(ZoomBut[InstruNo], 0);
			}
			break;
			
		case 27:
			if (GetControlHilite(SelectBut[InstruNo]) != 255 && MyTrackControl(SelectBut[InstruNo], theEvent.where, NULL)) {
		...
		case 18:
			if (GetControlHilite(ZoomBut[InstruNo]) != 255 && MyTrackControl(ZoomBut[InstruNo], theEvent.where, NULL)) {
		...
		case 25:
			if (MyTrackControl(InfoBut[InstruNo], theEvent.where, NULL)) {
		...
		case 28:
		{
			...
			for (i = 0, kk = 0; i < curMusic->fid[ InstruNo].numSamples; i++) {
				...
				if (curData->stereo) {
					...
					pStrcpy(str2, str);
					pStrcat(str2, "\p Left");
					AppendMenu(sMenu, str2);
					pStrcpy(str2, str);
					pStrcat(str2, "\p Right");
					AppendMenu(sMenu, str2);
				} else {
					menuSampleId[kk++] = i;
			...
			switch (curSample[InstruNo]) {
				...
				default:
					for (i = 0; i < 255; i++) {
						if (menuSampleId[i] == curSample[InstruNo])
							break;
					}
					i = 4 + menuSampleId[i];
					if (curMode[InstruNo])
			...
			mresult = PopUpMenuSelect(sMenu, Zone.v, Zone.h, i);
...
void DoKeyPressSample(DialogPtr	theDia, short theChar)
{
	...
	switch (theChar) {
		...
		case 0x1E: // --
			...
			ResetSelectionSample(CurWin);
			ZoomLevel[CurWin] = 1;
			SetControlValue(xScroll[CurWin], 0);
			SetControlH(CurWin);
		...
		case 0x1F: // ++
			if (curSample[CurWin] == volumeEnv) {
				...
				if (curMusic->fid[CurWin].numSamples > 0)
					curSample[CurWin] = 0;
					...
					default:
						...
						if (newSize == 0) {
							...
							if (newSnd != NULL) {
								...
								if (SampleDataD(CurWin)->loopBeg < 0)
									SampleDataD(CurWin)->loopBeg = 0;
		...
		case '*':
			...
			switch (SelectMode[CurWin]) {
				case eSelect:
					HiliteControl(SelectBut[CurWin], kControlButtonPart);
					break;
					
				case ePencil:
					HiliteControl(PencilBut[CurWin], kControlButtonPart);
					break;
					
				case eZoom:
					HiliteControl(ZoomBut[CurWin], kControlButtonPart);
					break;
			}
			break;
		...
		default:
			...
			if ((short)theChar >= 0 && (short)theChar < 300) {
				...
				if (i >= 0 && i < NUMBER_NOTES) {
					...
					if (TrackMem[TouchIn] < 0 || TrackMem[TouchIn] >= MADDriver->DriverSettings.numChn)
						TrackMem[TouchIn] = 0;
...
Boolean DragSample(RgnHandle myRgn, short theNo, EventRecord *theEvent)
{
	...
	if (SampleDataD(theNo)->amp == 8)
		ConvertInstrumentIn((Byte*)(*theSound + temp), inOutBytes);
	else
		ConvertInstrumentOut16((short*)(*theSound + temp), inOutBytes);
	//******************************************
	//******************************************
