...
static short AHelp[] = {HAZ, HLoad, HDel, HInfo, HopenA, HPref, HWind, HList, HWhere};
...
short MLAddRow(short no, short pos)
{
	...
	tt = myList.maxY  - (myList.rect.bottom - myList.rect.top) / myList.HCell;
	if (tt < 0)
		tt = 0;
...
void MLDelRow(short no, short pos)
{
	...
	myList.maxY -= no;
	if (myList.maxY < 0) MADDebugStr(__LINE__, __FILE__, "");
	PLSetControl(&myList);
	...
	*(short*) aCopy = myList.maxY*2;
	...
	for (theCell.v = 0 ; theCell.v < myList.maxY; theCell.v++) {
		...
		if (inOutBytes > MAXSTCSIZE) {
			MADDebugStr(__LINE__, __FILE__, "List too large for saving");
...
void SaveMODList(void)
{
	...
	if (DoCustomSave("\pSave Music List as:", curMusicList.name, 'STCf', &spec))
		return;
...
void OpenMODListSTCf(FSSpec spec)
{
	...
	if (DropZone >= 0)
		val = MLAddRow(theNo, DropZone);
	else
		val = MLAddRow(theNo, MAXITEM);
	...
	for (theCell.v = val, i = 1; theCell.v < theNo + val ; theCell.v++, i += 2) {
		...
		if (iErr == noErr)
			HGetVol(NULL, &myFSS.vRefNum, &myFSS.parID);
		else {
			myFSS.vRefNum = 0;
			myFSS.parID = 0;
		}
...
Boolean PLSearch(Ptr text, short size, short mode, Point *cell, PrivateList	*aL)
{
	...
	for (i = 0; i < aL->maxY; i++) {
		switch (mode) {
			case 1:
				if (!MyOMatchNextList((Ptr)(specList[i]->name + 1), text, specList[i]->name[0], size)) {
					cell->h = 0;
					cell->v = i;
...
void  AddMODList(Boolean ShowInfo, Str255 theString, short VRef, long parID)
{
	...
	if(whichCell != -1) {
		if (whichCell < 0) MADDebugStr(__LINE__, __FILE__, "AddModlist err");
		...
		if (DropZone >= 0)
			val = MLAddRow(1, DropZone);
		else
			val = MLAddRow(1, MAXITEM);
	...
	if (attributes & kDragInsideSenderWindow) {
		if (selectedControl == NULL) {
			if (DropZone >= 0) {
				theCell.v = 0;	theCell.h = 0;
				if (PLGetSelect(&theCell, &myList)) {
					short noSelec = myList.select.bottom - myList.select.top;
			...
			if (selectedControl == InfoBut) {
				...
				theCell.h = 0;
				if (PLGetSelect(&theCell, &myList))
					FileInformations(theCell.v);
			}
			if (selectedControl == DelBut)
				DoKeyPressMODList(8);
			else if (selectedControl == OpenBut)
...
Boolean NDragMusicFile(RgnHandle myRgn, EventRecord *theEvent, Point theCell, short noCell)
{
	...
	if (!(attributes & kDragInsideSenderApplication)) {
		...
		for (i = 0; i < noCell; i++)
			ReceiveNewFSSpec(&dropLocation, specList[theCell.v + i]);
...
void CreateMODListWindow(void)
{
	...
	InstallTrackingHandler(MyTrackingMODListUPP, GetDialogWindow(MODListDlog), NULL);
	InstallReceiveHandler(MyReceiveMODListUPP, GetDialogWindow(MODListDlog), NULL);
...
void ShowMusicInfo(Str255 name, short VRef, long parID, short id)
{
	...
	if (caRect.right == BIGMUSICLIST) {
		...
		if (OpenableFile(fndrInfo.fdType, &fss) == false &&
...
void DoChangeMODName(void)
{
	...
	NumToString(curMusic->header->tempo, theStr);
	SetDText(TheDia, 7, theStr);
	NumToString(curMusic->header->speed, theStr);
	...
	if (curMusic->header->showCopyright)
	...
	if (curMusic->header->XMLinear)
		TurnRadio(19, TheDia, true);
	else
		TurnRadio(19, TheDia, false);
	...
	if (itemHit == 1) {
		...
		if (GetControlValue((ControlHandle) itemHandle))
			curMusic->header->MODMode = true;
		else curMusic->header->MODMode = false;
		
		GetDialogItem(TheDia, 20, &itemType, &itemHandle, &itemRect);
		if (GetControlValue((ControlHandle) itemHandle))
			curMusic->header->MultiChan = true;
		else
			curMusic->header->MultiChan = false;
		
		GetDialogItem(TheDia, 19, &itemType, &itemHandle, &itemRect);
		if (GetControlValue((ControlHandle) itemHandle))
			curMusic->header->XMLinear = true;
		else
			curMusic->header->XMLinear = false;
		
		if (curMusic->header->MODMode)
			MADDriver->MODMode = true;
		else
			MADDriver->MODMode = false;
		
		if (curMusic->header->XMLinear)
			MADDriver->XMLinear = true;
		else
			MADDriver->XMLinear = false;
...
short FindTYPE(Str255 aStr, MenuHandle menu)
{
	...
	for (i = 1; i <= CountMenuItems(menu); i++) {
		...
		if (*((OSType*)(aStr + 1)) == *((OSType*)(bStr + 1)))
			return i;
	}
	...
	*((OSType*)(aStr + 1)) = fndrInfo.fdCreator;
	...
	if (MADMusicIdentifyFSp(gMADLib, tempC, &myFSS) != noErr) {
		...
		if (iErr == noErr) {
			...
			PathNameFromDirID(myFSS.parID, myFSS.vRefNum, aStr);
...
void DoLoadOtherMusic(Boolean Affiche)
{
	...
	if (thePrefs.LoopType == 4) // Will be handled later...
		return;
...
ReRandom:
	switch (thePrefs.LoopType) {
		case 0:			// repeat
			MADDebugStr(__LINE__, __FILE__, "What are you doing here??");
			return;
			break;
		...
		case 2:			// shuffle
			...
			if (theCell.v < 0 || theCell.v >= myList.maxY)
				theCell.v = 0;
			break;
	}
	...
	MADReset(MADDriver);
	if (thePrefs.AutoPlayWhenOpen && loaded)
		DoPlay();
...
OSErr GereMusicListChanged()
{
	...
	if (changedMusicList == false)
		return noErr;
	if (thePrefs.SaveMusicList == false)
		return noErr;
...
void DoNullMODList()
{
	...
	if (oldWindow == GetDialogWindow(MODListDlog)) {
