...
static	GWorldPtr		gGWorldAbout = 0L, gMaskWorld = 0L, gResultWorld = 0L, gFreePict = 0L;
...
Ptr		aPtr = 0L;
	...
	lBytesFree = CompactMem( FreeMem());
	lBytesFree = CompactMemSys( FreeMem());
...
void ShowPerformance(void)
{
	...
	aDialog = GetNewDialog( 158, 0L, (WindowPtr) -1L);
...
void DoAbout(void)
{
	...
	aDialog = GetNewDialog( 131, 0L, (WindowPtr) -1L);
	SetPortDialogPort( aDialog);
...
vScroll = NewControl( GetDialogWindow( theDialog), &itemRect, "\p", true, 0, 0, 0, gScrollBarID, 0L);
...
void DoContent(WindowPtr theWindow, EventRecord *theEventI)
{
	...
	case kControlIndicatorPart:
		...
		else TrackControl(theControl, theEventI->where, 0L);
...
pascal void ScrollProc (ControlHandle theControl, short theCode)
{
	...
	if( theCode != kControlIndicatorPart) SetCtlValue(theControl, oldCtl+scrollAmt);
