...
void SetUpScrollDigi(void)
{
	...
	if (gUseControlSize)
		SetControlViewSize(ControlDigi, (viewRect.right - viewRect.left) / TEXTLARG);
}
...
void DoNullDigiWindow(void)
{
	...
	for (x = 0; x < ITEMNO; x++) {
		...
		for (i = start; i < end; i ++) {
			switch (x) {
				...
				case 6:
					if (MADDriver->chan[i].maxPtr - MADDriver->chan[i].curPtr < 0)
						val = 0;
					else
						val = MADDriver->chan[i].maxPtr - MADDriver->chan[i].curPtr;
					NumToString(val, tempStr);
					break;
				...
				case 7:
					if (MADDriver->chan[i].maxPtr - MADDriver->chan[i].begPtr < 0)
						...
						val = MADDriver->chan[i].maxPtr - MADDriver->chan[i].begPtr;
					NumToString(val, tempStr);
					break;
				...
				case 11:
					if (MADDriver->chan[i].maxPtr > MADDriver->chan[i].curPtr)
						val = 1;
					else
						val = 0;
				...
				case 12:
					if (i < MADDriver->DriverSettings.numChn)
						val = 1;
					else
						val = 0;
			...
			tempRect.right += TEXTLARG;
			tempRect.left += TEXTLARG;
		}
		
		tempRect.left = viewRect.left;
		tempRect.right = tempRect.left + TEXTLARG;
		tempRect.top += TEXTHI;
		tempRect.bottom += TEXTHI;
	}
	...
	tempRect.top += TEXTHI * 3;
	tempRect.bottom += TEXTHI * 3;
	...
	tempRect.left += 2 * TEXTLARG;
	tempRect.right += 2 * TEXTLARG;
...
void  UpdateDigiListWindow(DialogPtr GetSelection)
{
	...
	TETextBox(tempStr + 1, tempStr[0], &aRect, teJustRight);
	aRect.top += TEXTHI * 3;
	aRect.bottom += TEXTHI * 3;
	
	/*** GŽnŽral Information ***/
	
	tempRect = viewRect;
	
	tempRect.top = aRect.top;
	...
	TETextBox(tempStr + 1, tempStr[0], &aRect, teJustRight);
	aRect.left += 2*TEXTLARG;
	aRect.right += 2*TEXTLARG;
