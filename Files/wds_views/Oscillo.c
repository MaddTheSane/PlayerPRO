...
void AdjustZoomOscillo2(Rect *vRect)
{
	...
	if (thePrefs.osciTile)
		tempB = OsciVStart + 15 + 1 * (InterText + OsciH);
	else
	...
	switch (MADDriver->DriverSettings.outPutBits) {
		...
		case 8:
			switch (MADDriver->DriverSettings.outPutMode) {
				case DeluxeStereoOutPut:
					if (LeftChannel)
						return Transformation8(MADDriver->OscilloWavePtr + 1);
					else
						return Transformation8(MADDriver->OscilloWavePtr);
					break;
			}
			break;
	}
...
Ptr GetQuicktimeChannel(Boolean LeftChannel, long Size)
{
	...
	if (LeftChannel)
		tempPtr = Transformation8(tempPtr + 1);
	else
		tempPtr = Transformation8(tempPtr);
...
void DrawOscilloscope2(OsciRec	*osciPtr, short no, Boolean lastCall)
{
	...
	if (osciPixMap[no] != NULL) {
		...
		if (thePrefs.OscilloLine) {
			C8BitOsciPixMapLine((Byte*) GetAudioSource(realno) + OsciOffSet,
		...
		if (OsciType == OutPutAudio) {
			if (realno != 0)
				soundlevel = MADDriver->levelR;
			else
				soundlevel = MADDriver->levelL;
			...
			if (realno != 0) {
				...
				if (soundlevel > maxLevelR) {
					maxLevelR = soundlevel;
					timeLevelR = 60;
				}
				...
				if (soundlevel > maxLevelL) {
					maxLevelL = soundlevel;
					timeLevelL = 60;
				}
...
{
	Rect   		inRect, caRect, tempRect, bRect;
	GrafPtr		SavePort;
	short		i, OsciSee;
		...
		case 5:
			...
			if (HiWord(mresult) != 0) {
				switch (LoWord(mresult) - 1) {
					...
					case TrackAudio:
						OsciType = TrackAudio;
						if (MicroPhone == true && SpectrumMicrophone == false)
							MicroOff();
						OscilloMicrophone = false;
