...
//static	Rect					selecZone, PatRectList;
//static	ControlHandle			xScroll, yScroll;
//static	short					HCell, LCell;
...
void ConvertPointAndType( Point *cPt, short *Type)
{
	...
	cPt->h += GetCtlValue( myList.xScroll);	cPt->v += GetCtlValue( myList.yScroll);

	// Type Identifier
	
	dist -= myList.rect.left + (cPt->h - GetCtlValue( myList.xScroll)) * myList.LCell;
...
unsigned long	ttt;
	...
	if( EditorDlog == 0L) CreatePartitionWindow();
...
{
	if( GetCtlMax( myList.xScroll) != curMusic->header->numChn - (myList.rect.right - myList.rect.left) / myList.LCell)
	{
		DrawEditorUp();
		PLSetControl( &myList);
	}
...
Rect			aR;
	...
	if( EditorDlog == 0L) return;
	
	GetPort( &SavePort);
 	SetPortDialogPort( EditorDlog);
	
	aR.left = myList.rect.left + (track - GetCtlValue( myList.xScroll)) * myList.LCell;		aR.right = aR.left + myList.LCell;
	aR.top = myList.rect.top + (pos - GetCtlValue( myList.yScroll)) * myList.HCell;		aR.bottom = aR.top + myList.HCell;
...
short	temp;
	...
	temp = ReaderCopy - GetCtlValue( myList.yScroll);
...
//	CurRect.right = EditorDlog->portRect.right - 15;	//LEFTBORD;
	...
	if( CurRect.right > myList.rect.left + ( myList.maxX - GetCtlValue( myList.xScroll)) * myList.LCell)
		CurRect.right = myList.rect.left + ( myList.maxX - GetCtlValue( myList.xScroll)) * myList.LCell;
	...
	for( i = GetCtlValue( myList.yScroll); i < PLGetMaxYValue( &myList); i++)
	{
		if( thePrefs.UseMarkers)
		{
			if( i >= thePrefs.MarkersOffSet)
			{
				...
				tRect.top = myList.rect.top + (i - GetCtlValue( myList.yScroll)) * myList.HCell;
...
ThemeDrawingState	state;
	...
	for( i = 0; i < PLGetMaxXValue( &myList) - GetCtlValue( myList.xScroll); i++)
	{
		...
		NumToString( i + 1 + GetCtlValue( myList.xScroll), aStr);
		
		MyTETextBox( &thePrefs.tracksColor[ i + GetCtlValue( myList.xScroll)], &tempRect, aStr, MADDriver->Active[ i]);
	...
	if( ReaderCopy != MADDriver->PartitionReader)
	{
		...
		if( MusicPlayActive == true)
		{
			...
			if( ReaderCopy >= PLGetMaxYValue( &myList) ||
				ReaderCopy < GetCtlValue( myList.yScroll))
				{
...
Str255	aStr;
	...
	for( i = GetCtlValue( myList.yScroll); i < PLGetMaxYValue( &myList); i++)
	{
		MoveTo( 2, (i - GetCtlValue( myList.yScroll)) * myList.HCell + myList.rect.top + 8);
...
short	i, temp, off;
	...
	tRect = myList.rect;
	if( tRect.right > tRect.left + ( myList.maxX - GetCtlValue( myList.xScroll)) * myList.LCell)
		tRect.right = tRect.left + ( myList.maxX - GetCtlValue( myList.xScroll)) * myList.LCell;
	
	off = GetCtlValue( myList.yScroll);
...
Rect			caRect;
	...
	if( thePrefs.UseMarkers) DrawMarkers( GetCtlValue( myList.yScroll), PLGetMaxYValue( &myList));
	else EraseRect( &myList.rect);
	
	xPos = myList.rect.left + 4;
	for( x = GetCtlValue( myList.xScroll); x < PLGetMaxXValue( &myList); x++)
	{
		DrawGrayLines( xPos);
		...
		CmdPtr	=	GetMADCommand( GetCtlValue( myList.yScroll), x, curMusic->partition[ CurrentPat]);
...
Rect		eraseRect, tRect;
	...
	{
		SetRect( &eraseRect, myList.rect.left, myList.rect.top + (startY - GetCtlValue( myList.yScroll)) * myList.HCell, myList.rect.right, myList.rect.bottom);
	}
	else
	{
		SetRect( &eraseRect, myList.rect.left, myList.rect.top + (startY - GetCtlValue( myList.yScroll)) * myList.HCell, myList.rect.right, myList.rect.top + (startY - GetCtlValue( myList.yScroll)) * myList.HCell + (endY - startY)*myList.HCell);
	}
	...
	for( x = GetCtlValue( myList.xScroll); x < PLGetMaxXValue( &myList); x++)
	{
		...
		yPos	=	myList.rect.top + 9 + (startY - GetCtlValue( myList.yScroll)) * myList.HCell;
...
//		RGBBackColor( &theColor);
		...
		GetPortVisibleRegion( GetDialogPort( EditorDlog), visibleRegion);
 		...
 		UpdtDialog( EditorDlog, visibleRegion);
...
//	if( (modifiers & cmdKey) != 0 && (modifiers & optionKey) != 0)
	...
	if( modifiers == thePrefs.FastDigitalEdition)	//& thePrefs.FastDigitalEdition
	{
		...
		switch( type)
		{
			...
			case NoteTE:
				InsMenuItem( NoteMenu, "\p000", NUMBER_NOTES);
				InsMenuItem( NoteMenu, "\pOFF", NUMBER_NOTES+1);
				...
				DelMenuItem( NoteMenu, NUMBER_NOTES + 1);
				DelMenuItem( NoteMenu, NUMBER_NOTES + 1);
			break;
...
switch( whichItem)
{
	...
	if( GetControlHilite( FillBut) == 0)	// && )
	{
		...
		MyTrackControl( FillBut, theEvent.where, 0L);
	...
	case 39:
	//	RGBBackColor( &theColor);
		if(MyTrackControl( (ControlHandle) CheckBut[ whichItem - 37], theEvent.where, 0L))
		{
			SetCtlValue( (ControlHandle) CheckBut[ whichItem - 37], !GetCtlValue( (ControlHandle) CheckBut[ whichItem - 37]));
	...
	case 30:
	{
		...
		//	BackColor( whiteColor);
			...
			OnOffVolume = true;
			SetCtlValue( (ControlHandle) CheckBut[ 2], OnOffVolume);
		}
	...
	case 31:
	{
		...
		//	BackColor( whiteColor);
			...
			OnOffArg = true;
			SetCtlValue( (ControlHandle) CheckBut[ 1], OnOffArg);
		}
		DeleteMenu( GetMenuID( curMenu));
		DisposeMenu( curMenu);
	}
	...
	case 32:
	{
		...
		//	BackColor( whiteColor);
			...
			OnOffFX = true;
			SetCtlValue( (ControlHandle) CheckBut[ 0], OnOffFX);
		}
		DeleteMenu( GetMenuID( EffectMenu));
	}
	break;
	...
	case 17:
	{
		...
		//	BackColor( whiteColor);
			...
			OnOffInstru = true;
			SetCtlValue( (ControlHandle) CheckBut[ 3], OnOffInstru);
		}
		DeleteMenu( GetMenuID( InstruMenu));
		DelMenuItem( InstruMenu, 1);
	}
	...
	case 13:
		if(MyTrackControl( TraceBut, theEvent.where, 0L))
		{
	...
	case 14:
		if(MyTrackControl( FindBut, theEvent.where, 0L))
		{
			HandleOtherChoice( 6);
		}
	break;
	...
	case 23:
		if( GetControlHilite( DownBut) == 0  && MyTrackControl( DownBut, theEvent.where, 0L))
		{
			DoKeyPressEditor( '/');
		}
	break;
	...
	case 10:
		if( GetControlHilite( UpBut) == 0  && MyTrackControl( UpBut, theEvent.where, 0L))
		{
			DoKeyPressEditor( '*');
		}
	break;
	...
	case 11:
		...
		if(MyTrackControl( RecBut, theEvent.where, 0L))
		{
	...
	case 8:
		if( GetControlHilite( ShowBut) == 0  && MyTrackControl( ShowBut, theEvent.where, 0L))
		{
			theCell.v = theCell.h = 0;
			if( PLGetSelect( &theCell, &myList))
			{
	...
	case 5:
		...
		else if( myPt.v < myList.rect.top)
		{
			...
			theCell.h += GetCtlValue( myList.xScroll);
...
void CreatePartitionWindow(void)
{
	...
	EditorDlog = GetNewDialog( 139, 0L, GetDialogWindow( ToolsDlog));
	SetWindEtat( GetDialogWindow(EditorDlog));
	...
	SetControlTitle( CheckBut[ 3], "\pIns: 000");		SetCtlValue( CheckBut[ 3], true);
	...
	if( DragManagerUse)
	{
		...
		InstallTrackingHandler( (DragTrackingHandlerUPP) MyTrackingHandlerUPP, GetDialogWindow( EditorDlog), (void *) 0L);
		InstallReceiveHandler( (DragReceiveHandlerUPP) MyReceiveDropHandlerUPP, GetDialogWindow( EditorDlog), (void *) 0L);
	}
...
Ptr ConvertPcmd2Text( Pcmd *myPcmd)
{
	...
	if( myText == 0L) MyDebugStr( __LINE__, __FILE__, "Memory Error");
	strcpy( myText, "");
...
Pcmd* CreatePcmdFromSelection(void)
{
	...
	theCell.h = theCell.v = 0;
	if( !PLGetSelect( &theCell, &myList)) return 0L;
	...
	switch (message)
	{
		...
		case kDragTrackingInWindow:
			...
			if( PtInRect( localMouse, &myList.rect))
			{
				...
				tempRect.left = myList.rect.left + 1 + (CellSelec.h - GetCtlValue( myList.xScroll)) * myList.LCell;
				tempRect.top = myList.rect.top + (CellSelec.v - GetCtlValue( myList.yScroll)) * myList.HCell;						
	...
	//	HSetVol( 0L, target.vRefNum, target.parID);
		...
		else
		{
			if( theFlavor == 'VCT5')
			{
				...
				if( CreateAIFFExporting( true, fRefNum, &target, 'AIFF', 0L))
				{
...
Boolean DragEditor( RgnHandle myRgn, Pcmd	*myPcmd, EventRecord *theEvent)
{
	...
	if( thePrefs.editorSoundDrag)
	{
		...
		AddDragItemFlavor( theDrag, 1, 'VCT5', 0L, 0, 0);
	}
	else
	{
