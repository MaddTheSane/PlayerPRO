...
short	ret;
	...
	ret = 1 + GetCtlValue( xScroll) + (StaffRect.right - StaffRect.left) / XSize;
...
short	ret;
	...
	ret = 1 + GetCtlValue( yScroll) + (StaffRect.bottom - StaffRect.top) / YSize;
...
void CreateCurStaffRect()
{
	...
	{
		CurRect.top = StaffRect.top + (startYSelec - GetCtlValue( yScroll))*YSize;
		CurRect.bottom = StaffRect.top + (endYSelec - GetCtlValue( yScroll))*YSize + YSize;
		
		CurRect.left = StaffRect.left + (startXSelec - GetCtlValue( xScroll))*XSize;
		CurRect.right = StaffRect.left + (endXSelec - GetCtlValue( xScroll))*XSize + XSize;
	}
}
...
void ConvertPt2Note( Point pt, short *position, short *track, short *note)
{
	...
	if( position != 0L) 	*position = pt.h;
	if( track != 0L) 		*track = pt.v;
}
...
Boolean aNoteStaff( Point pt, short *whichNote)
{
	...
	if( whichNote != 0L) *whichNote = note;
...
void DrawNoteCarre( Point *myPt)
{
	...
	{
		ConvertPt2Note( *myPt, 0L, 0L, &note);
...
void DoNullStaff(void)
{
	...
	if( ReaderCopy != MADDriver->PartitionReader)
	{
		...
		{
			if( ReaderCopy > GetMaxXStaff() || ReaderCopy < GetCtlValue( xScroll))
			{
				SetCtlValue( xScroll, ReaderCopy);
				
				MakeUpdate = true;
			}
			else DrawStaffReader();
		}
	}
	...
	if( oldWindow == GetDialogWindow( StaffDlog))
	{
		...
		if( PtInRgn( myPt, visibleRegion))
		{
			...
			//	temp = (myPt.v - StaffRect.top) - YSize*temp;
				...
				else*/
				{
					...
					else
					{
						...
						{
							case noteM:
								if( aNoteStaff( myPt, 0L)) SetCursor( &HandCrsr);
			...
			else
			{
				...
				if( PtInRect( myPt, &tempRect)) SetCursor( &PlayCrsr);
				else
				{
...
void DrawStaffReader()
{
	...
	tRect.left	= StaffRect.left + (ReaderCopy - GetCtlValue( xScroll)) * XSize + XSize/2 - (add*XSize)/2;
...
void EraseStaffReader()
{
	...
	{
		if( ReaderCopy < thePrefs.MarkersOffSet) ForeColor( whiteColor);
		else
		{
...
void DrawStaffFrame()
{
	...
	for( i = GetCtlValue( xScroll); i < GetMaxXStaff(); i+= add)
	{
		tRect.left	= StaffRect.left + (i - GetCtlValue( xScroll)) * XSize + XSize/2 - (add*XSize)/2;
		tRect.right	= tRect.left + (add*XSize);
	...
	for( i = GetCtlValue( yScroll); i < GetMaxYStaff(); i++)
	{
		Str255		str;
		short		add;
		...
		tRect.top	= StaffRect.top + 1 + (i - GetCtlValue( yScroll)) * YSize;
		...
		if( thePrefs.TempsNum > 9 || thePrefs.TempsUnit > 9) add = -8;
		else add = 0;
		
		MoveTo( tRect.left + 32 + add, tRect.top + 23);	NumToString( thePrefs.TempsNum, str);		DrawString( str);
		MoveTo( tRect.left + 32 + add, tRect.top + 40);	NumToString( thePrefs.TempsUnit, str);		DrawString( str);
		
		MoveTo( tRect.left + 32 + add, tRect.top + 65);	NumToString( thePrefs.TempsNum, str);		DrawString( str);
		MoveTo( tRect.left + 32 + add, tRect.top + 82);	NumToString( thePrefs.TempsUnit, str);		DrawString( str);
...
void UpdateStaffNote( short track)
{
	...
	aR.top		= StaffRect.top + 1 + (track - GetCtlValue( yScroll)) * YSize;
	aR.bottom	= aR.top + YSize;
	
	if( SectRect( &aR, &StaffRect, &aR)) InvalWindowRect( GetDialogWindow( StaffDlog), &aR);
...
void DrawStaffNotes()
{
	...
	saveClipRgn = NewRgn();
 	...
 	for( x = GetCtlValue( yScroll); x < GetMaxYStaff(); x++)
	{
		tRect = StaffRect;
		
		tRect.top		= StaffRect.top + 1 + (x - GetCtlValue( yScroll)) * YSize;
		...
		LineTo( tRect.right, tRect.bottom-1);
 		...
 		{
 			tRect.left	= StaffRect.left + (i - GetCtlValue( xScroll)) * XSize;
 		...
 		tRect.top		= StaffRect.top + (x - GetCtlValue( yScroll)) * YSize;
		...
		//	else RGBForeColor( &Gray);
		 	...
		 	for( i = 0; i < GetMaxXStaff() ;)
			{
				...
				for(;; i++)
				{
					...
					if( cmd->note != 0xFF && cmd->note != 0xFE) { note = cmd;		break; }
					if( i >= curMusic->partition[ CurrentPat]->header.size) break;
				}
				startNote = i;
				i++;
				...
				if( note)
				{
					for(;; i++) // Scan pour la fin de la note - NOTE OFF
					{
						...
						if( cmd->note != 0xFF) break;
						if( i >= curMusic->partition[ CurrentPat]->header.size) break;
					}
					endNote = i;
				}
				
				if( startScan < startNote && startNote >= GetCtlValue( xScroll))		// Ajout d'un SILENCE
				{
					CIconHandle	silIcon;
					...
					while( silenceLength > 0)
					{
						...
						if( silenceLength > TempsSize) silenceLength = TempsSize;
						
						tRect.left		= StaffRect.left + BEMOLSIZE/2 + (startNote - silenceLengthCopy - GetCtlValue( xScroll)) * XSize;
						
								if( silenceLength >= 16) { silIcon = silence[ 0];	remove = 16; }
						else	if( silenceLength >=  8) { silIcon = silence[ 1];	remove = 8;  }
						...
						{
							if( silenceLength > 0) goto SilenceEncore;
						}
				...
				if( note != 0L && endNote >= GetCtlValue( xScroll))
				{
					if( note->note != 0xFF && note->note != 0xFE)
					{
						...
						while( silenceLength > 0)
						{
							...
							tRect.left		= StaffRect.left + (endNote - silenceLengthCopy - GetCtlValue( xScroll)) * XSize;
							
									if( silenceLength >= 16) { silIcon = noteIcn[ 0];				remove = 16; }
							else	if( silenceLength >= 12) { silIcon = noteIcn[ 1]; P1 = true;	remove = 12; }
							else	if( silenceLength >=  8) { silIcon = noteIcn[ 1];				remove = 8;  }
							else	if( silenceLength >=  6) { silIcon = noteIcn[ 2]; P1 = true;	remove = 6;  }
							else	if( silenceLength >=  4) { silIcon = noteIcn[ 2];				remove = 4;  }
							...
							if( BlackWhite[ note->note%12])
							{
								if( first)
								{
									if( prevNote == -1) demitonID = 2;
									else if( prevNote <= note->note) demitonID = 2;
									else if( prevNote > note->note) demitonID = 1;
								}
								
								if( demitonID == 2) PlotCIcon( &tRect, demitonIcon[ 2]);
								else if( demitonID == 1)
								{
									tRect.top		= StaffRect.top + 1 + (x - GetCtlValue( yScroll)) * YSize;
							...
							if( silIcon == 0L) MyDebugStr( __LINE__, __FILE__, "");
							if( ptIcon == 0L) MyDebugStr( __LINE__, __FILE__, "");
							
							tRect.right = tRect.left + (*silIcon)->iconPMap.bounds.right;
							
							if( silIcon)	PlotCIcon( &tRect, silIcon);
							if( P1)			PlotCIcon( &tRect, ptIcon);
							...
							if( note->note >= 60 && note->note <= 61)
							{
								...
								short	yy = StaffRect.top + 1 + (x - GetCtlValue( yScroll)) * YSize;
							...
							if( silenceLength != silenceLengthCopy)
							{
								if( silenceLength > 0) goto NoteEncore;
							}
...
{ 
	...
	tempRect = StaffRect;
	tempRect.left = 0;
	tempRect.top = StaffRect.top + 1 + (curMusic->header->numChn - GetCtlValue( yScroll)) * YSize;
	if( SectRect( &tempRect, &StaffRect, &tempRect)) EraseRect( &tempRect);
...
void CalculDiffStartStaff(void)
{
	...
	ConvertPt2Note( myPt, &pos, &track, 0L);
...
void DoItemPressStaff( short whichItem, DialogPtr whichDialog)
{
		...
		if (theEvent.what == mouseDown)
		{
				...
				else
				{
					switch( mode)
					{
						...
						else	newSelec:
						{
							...
							ConvertPt2Note( myPt, 	&start.h, &start.v, 0L);
							...
							do
							{
								...
								ConvertPt2Note( curPt, 		&aRect.left, &aRect.top, 0L);
			...
			else
			{
				...
				if( PtInRect( myPt, &tempRect))
				{
					...
					ConvertPt2Note( tPt, 0L, &track, &note);
	...
	{
		case 21:
			 	if( MyTrackControl( saveBut, theEvent.where, 0L))
		...
		case 23:
			if( GetControlHilite( FXBut) == 0 && MyTrackControl( FXBut, theEvent.where, 0L))
			{
				short	itemType;
				Rect	itemRect;
				Handle	itemHandle;
...
/*		case 21:
		...
		case 10:
		{
			...
			InsMenuItem( InstruMenu, "\pNo Ins", 0);
...
curVal = sVal = GetCtlValue( theControl);
	...
	switch( ctlPart)
	{
		...
		case kControlPageUpPart:
			...
			{
				case yScrollNum:	curVal -= GetMaxYStaff() - GetCtlValue( yScroll) - 1;	break;
				case xScrollNum:	curVal -= GetMaxXStaff() - GetCtlValue( xScroll) - 1;	break;
			}
...
void CreateStaffWindow(void)
{
	...
	Handle			itemHandle;
	short			itemType, i, itemHit, temp, iWidth;
	Point			cSize;
	...
	StaffDlog = GetNewDialog( 134, 0L, (WindowPtr) ToolsDlog);
	SetWindEtat( GetDialogWindow(StaffDlog));
...
//	TextFont( 4);	TextSize( 9);
	...
	if( DragManagerUse)
	{
		...
		InstallTrackingHandler( (DragTrackingHandlerUPP) MyTrackingHandlerUPP, GetDialogWindow( StaffDlog), (void *) 0L);
		InstallReceiveHandler( (DragReceiveHandlerUPP) MyReceiveDropHandlerUPP, GetDialogWindow( StaffDlog), (void *) 0L);
	}
}
...
{
	Point 				theCell, aCell;
	Rect				caRect, cellRect, eRect, sRect;
	Str255				theStr;
	short				theNo, temp;
	Cmd					*theCommand;
	GrafPtr				SavePort;
	...
	if( theChar == deletekey) // Delete
 	...
 	else if( theChar ==  9)
 	{
		...
		switch( mode)
		{
			case selecM:	HiliteControl( selecBut, kControlButtonPart);	break;
	...
	switch (message)
	{
		...
		case kDragTrackingInWindow:
			...
			if (attributes & kDragInsideSenderWindow)
			{
				...
				if( PtInRect( localMouse, &coRect) && selectedControl == 0L) { HiliteControl( saveBut, kControlButtonPart);		selectedControl = saveBut;}
			...
			if( PtInRect( localMouse, &StaffRect))
			{
				...
				tempRect.top		= StaffRect.top + (track - GetCtlValue( yScroll)) * YSize;
				...
				tempRect.left = StaffRect.left + (pos - GetCtlValue( xScroll)) * XSize;
				tempRect.right = tempRect.left + PcmdLength * XSize;
				
				DropX		= pos;
				DropY		= track;
				if (attributes & kDragInsideSenderWindow) DropNote	= note;
				else DropNote = -1;
...
pascal OSErr MyReceiveDropStaff(WindowPtr theWindow, void* handlerRefCon, DragReference theDrag)
{
	...
	if( DropX == -1 && DropY == -1 && selectedControl == 0L) return( dragNotAcceptedErr);
	...
	if( attributes & kDragInsideSenderWindow)
	{
		if( selectedControl != 0L)
		{
			HiliteControl( selectedControl, 0);
			
			if( selectedControl == saveBut)		SavePcmdFile( CreatePcmdFromSelectionStaff());
	...
	{	
		...
		if( myPcmd != 0L)
		{
			...
			{
				// La source est dŽjˆ selectionnŽe...
			...
			startXSelec = DropX;				startYSelec = DropY;
...
void COPYStaff(void)
{
	...
	ZeroScrap();
	PutScrap( GetPtrSize( (Ptr) myPcmd), 'Pcmd', (Ptr) myPcmd);
	...
	#if MACOS9VERSION
	PutScrap( GetPtrSize( myText), 'TEXT', myText);
	#else
...
OSErr				anErr;
	...
	#if MACOS9VERSION
	lCntOrErr = GetScrap( 0L, 'Pcmd', &scrapOffset);
	#else
	lCntOrErr = 0;
	anErr = GetCurrentScrap( &scrap);
	...
	if( lCntOrErr > 0)
	{
		...
		#if MACOS9VERSION
		GetScrap( theHandle, 'Pcmd', &scrapOffset);
		#else
		HLock( theHandle);
