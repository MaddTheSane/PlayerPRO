...
static	short					oldPos = 0, oldPat = 0, oldTrack = 0, curPos = 0;
		TEHandle				TEH[6];
static	ControlHandle			keyMapCntl, DelBut, AllBut, PlayCntl, FlipBut;
...
void SwitchSize()
{
	...
	if (caRect.bottom == SMALLH)
		MySizeWindow(ToolsDlog, caRect.right, LARGEH, true);
	else {
...
void UpdatePatternMenu(void)
{
	...
	for (i = 0; i < x; i++)
		DeleteMenuItem(thePatternMenu, 1);
...
void DoNullCmdWindow(void)
{
	...
	if (thePrefs.MusicTrace == true || curPos == -1) {
		if (oldPos != MADDriver->PartitionReader)
			UpdateNow = true;
	...
	if (UpdateNow == true) {
		...
		oldPos = newVal;
		if (oldPos >= curMusic->partition[oldPat]->header.size)
			oldPos = curMusic->partition[oldPat]->header.size - 1;
		...
		myTESetText(EInstru[theCommand->ins], 3, TEH[InstruTE]);
		myTESetText(ENote[theCommand->note], 3, TEH[NoteTE]);
		myTESetText(&EEffect[theCommand->cmd], 1, TEH[EffectTE]);
		myTESetText(EArgu[theCommand->arg], 2, TEH[ArguTE]);
		
		if (theCommand->vol == 0xFF)
			myTESetText(EInstru[0], 2, TEH[VolumeTE]);
		else
			myTESetText(EArgu[theCommand->vol], 2, TEH[VolumeTE]);
		
		GetPortBounds(GetDialogPort(ToolsDlog), &caRect);
		
		if (caRect.right > 240)
			AfficheGoodEffect(theCommand->cmd, theCommand->arg);
		...
		case 9:
			if ((*TEH[curActif])->active != 0) {
				...
				while (next == true && loop-- >= 0) {
					...
					if (curActif > VolumeTE)
						curActif = InstruTE;
					if (curActif < InstruTE)
						curActif = VolumeTE;
					
					next = false;
					
					if (curActif == InstruTE && thePrefs.DigitalInstru == false)
						next = true;
					if (curActif == NoteTE && thePrefs.DigitalNote == false)
						next = true;
					if (curActif == EffectTE && thePrefs.DigitalEffect == false)
						next = true;
					if (curActif == ArguTE && thePrefs.DigitalArgu == false)
						next = true;
					if (curActif == VolumeTE && thePrefs.DigitalVol == false)
						next = true;
				}
			} else {
				curActif = InstruTE;
