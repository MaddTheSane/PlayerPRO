...
void CreateToolsDlog(void)
{
	...
	ToolsDlog = GetNewDialog( 143, 0L, (WindowPtr) -1L);
...
/*	GetDialogItem( ToolsDlog , 7, &itemType, &itemHandle, &itemRect);
	...
	if( DragManagerUse)
	{
		...
		InstallTrackingHandler( (DragTrackingHandlerUPP) MyTrackingToolsUPP, GetDialogWindow( ToolsDlog), (void *) 0L);
		InstallReceiveHandler( (DragReceiveHandlerUPP) MyReceiveToolsUPP, GetDialogWindow( ToolsDlog), (void *) 0L);
	}
...
void ScanTime()
{
	...
	{
		if( TimeScanPtr[ i] != 0L) DisposePtr( (Ptr) TimeScanPtr[ i]);
...
void DoItemPressTools( short whichItem, DialogPtr whichDialog)
{
	...
	{
	case 2:
		if( MyTrackControl( JumpBeforeCntl, theEvent.where, 0L))
		{
			DoSearchDown();
		}
	break;
	...
	case 44:
		if( MyTrackControl( RecordCntl, theEvent.where, 0L))
		{
			DoRecord();
		}
	break;
	...
	case 46:
		if( MyTrackControl( LoopCntl, theEvent.where, 0L))
		{
			DoChangeLoop();
		}
	break;
	...
	case 4:
		if( MyTrackControl( stopCntl, theEvent.where, 0L))
		{
			DoPause();
		}
	break;
	...
	case 7:
		if( MyTrackControl( JumpNextCntl, theEvent.where, 0L))
		{
			DoSearchUp();
		}
	break;
