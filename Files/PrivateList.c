...
curVal = sVal = GetCtlValue( theControl);
	...
	switch( ctlPart)
	{
		...
		case kControlPageUpPart:
			switch( lRefCon)
			{
				case yScrollNum:	curVal -= PLGetMaxYValue( myIntList) - GetCtlValue( myIntList->yScroll) - 1;	break;
				case xScrollNum:	curVal -= PLGetMaxXValue( myIntList) - GetCtlValue( myIntList->xScroll) - 1;	break;
			}
...
void PLAutoScroll( PrivateList	*aL)
{
	...
	else
	{
		...
		{
			Rect		aa = aL->rect;
			short	prevVal = GetCtlValue( aL->yScroll);
			...
			PLScrollInt( GetCtlValue( aL->yScroll), prevVal, GetCRefCon( aL->yScroll), aL);
		}
		
		if( XDist < aL->rect.left || XDist > aL->rect.right - aL->LCell)
		{
			Rect		aa = aL->rect;
			short	prevVal = GetCtlValue( aL->xScroll);
			...
			PLScrollInt( GetCtlValue( aL->xScroll), prevVal, GetCRefCon( aL->xScroll), aL);
		}
	}
}
...
Point			cPt, bPt = { -1, -1}, theCell;
Point			curPt, oldPt;
Boolean		ddTime = false;
...
GrafPtr	savePort;
	...
	switch( theChar)
	{
		...
		case 0x04:	// end
			PLSetMyIntList( aL);
			sVal = GetCtlValue( aL->yScroll);

			PLSetCtlValue( aL->yScroll, GetCtlMax( aL->yScroll));
			PLScrollInt( GetCtlMax( aL->yScroll), sVal, GetCRefCon( aL->yScroll), aL);
		break;
		...
		case 0x01:	// home
			...
			PLSetCtlValue( aL->yScroll, GetCtlMin( aL->yScroll));
			PLScrollInt( GetCtlMin( aL->yScroll), sVal, GetCRefCon( aL->yScroll), aL);
		break;
	}
