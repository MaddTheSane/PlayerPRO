...
Byte	SwitchColorLV( short i)
{
	...
	if( gGWorld == 0L) MyDebugStr( __LINE__, __FILE__, "gGWorld = 0L");
	
	GetGWorld( &oldPort, &oldGDeviceH);
	SetGWorld( gGWorld, 0L);
...
void SetControlOscillo(void)
{
	...
	if( thePrefs.osciTile) SetCtlMax( VControl, 0);
	else SetCtlMax( VControl, OsciNo - (caRect.bottom - 15 - OsciVStart) / (OsciH + InterText));
...
inline void  Bresenham(long x1, long y1, long x2, long y2, long row, Byte color, Ptr src)
{
...
void CreateOscilloWindow(void)
{
	...
	if( ValSaveData == 0L) return;

	GetPort( &savePort);

	for( i = 0 ; i < MAXTRACK; i++) osciPixMap[ i] = 0L;
	CurrentQuickPixMap = 0L;
	
	SetItemMark( ViewsMenu, mOscilloV, checkMark);

	OscilloDlog = GetNewDialog( 128, 0L, GetDialogWindow( ToolsDlog));
...
void CloseOscillo(void)
{
	...
	if( OscilloDlog != 0L) 
	{
		...
		DisposDialog( OscilloDlog);
		
		for( i = 0; i < MAXTRACK; i++)
		{
			if( osciPixMap[ i] != 0L) ZapPixMap( &osciPixMap[ i]);
		}
		if( CurrentQuickPixMap != 0L) DisposePtr( (Ptr) CurrentQuickPixMap);
